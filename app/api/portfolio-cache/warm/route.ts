...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabase
    ...
    .select("ticker")
    .not("ticker", "is", null)
    .limit(Number(process.env.PORTFOLIO_CACHE_WARM_HOLDING_LIMIT ?? 250));
  ...
  ).slice(0, Number(process.env.PORTFOLIO_CACHE_WARM_TICKER_LIMIT ?? 100));

  await runInBatches(
    tickers,
    Number(process.env.PORTFOLIO_CACHE_WARM_BATCH_SIZE ?? 8),
    async (ticker) => {
