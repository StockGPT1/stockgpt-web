...
function buildPortfolioSummary({
  ...
  const holdingsValue = holdingSummaries.reduce(
    (sum, holding) => sum + holding.currentValue,
    0,
  );
  const cashBalance = toNumber(portfolio.cash_balance, 0);
  const totalValue = holdingsValue + cashBalance;
  const costBasis = holdingSummaries.reduce(
    (sum, holding) => sum + holding.shares * holding.entryPrice,
    0,
  );
  ...
  const sectorCount = new Set(
    holdingSummaries.map((holding) => holding.sector).filter(Boolean),
  ).size;
  const weakCount = holdingSummaries.filter(
    (holding) =>
      (holding.score != null && holding.score < 5200) ||
      (holding.rank != null && holding.rank > 300),
  ).length;
...
export default async function Home() {
  ...
  if (user) {
    const { data: portfolioData } = await supabase
      ...
      .select(
        "id, name, risk_tolerance, time_horizon, investment_amount, cash_balance, currency, created_at",
      )
      .eq("user_id", user.id)
      .is("archived_at", null)
  ...
  return (
    <AppShell activePath="/dashboard">
      <main className="min-h-full overflow-visible lg:h-full lg:min-h-0 lg:overflow-hidden">
        <div className="grid gap-3 lg:h-full lg:min-h-0 lg:grid-cols-[minmax(0,1fr)_340px] lg:gap-3">
          <section className="grid gap-3 lg:h-full lg:min-h-0 lg:grid-rows-[150px_68px_176px_minmax(0,1fr)] lg:overflow-hidden">
            ...
            <div className="grid min-w-0 gap-3 lg:grid-cols-[minmax(0,1.05fr)_minmax(280px,0.95fr)] lg:overflow-hidden">
            ...
            <div className="overflow-hidden rounded-2xl bg-[#faf6f0] text-[#072116] shadow-[0_18px_42px_rgba(0,0,0,0.22)] ring-1 ring-white/20 lg:min-h-0">
              <div className="flex h-[66px] items-start justify-between gap-3 border-b border-[#072116]/10 px-4 py-3">
                ...
                <Link
                  ...
                  className="mt-1 shrink-0 rounded-full bg-[#ddb159] px-4 py-2 text-[10px] font-black text-[#072116] shadow-[0_6px_16px_rgba(221,177,89,0.26)] transition hover:brightness-105"
                >
              ...
              <RankingsLock isLocked={rankingsLocked} className="lg:hidden">
                <div>
                  ...
                  <div className="divide-y divide-[#072116]/8">
                    {rankings.length > 0 ? (
                      rankings.map((stock) => (
                        <RankingMobileRow key={stock.id} stock={stock} dailyMove={dailyMoveMap.get(stock.ticker ?? "")?.changePct} />
                      ))
                    ) : (
              ...
              <RankingsLock isLocked={rankingsLocked} className="hidden h-[calc(100%-66px)] min-h-0 overflow-hidden lg:block">
                <div className="flex h-full min-h-0 flex-col overflow-hidden">
                  <div className="min-h-0 flex-1 overflow-hidden">
                    {rankings.length > 0 ? (
                      rankings.map((stock) => {
                        ...
                        return (
                          <Link
                            ...
                            className={`group grid ${dashboardRankingsGrid} h-[10%] min-h-0 items-center border-b border-[#072116]/8 text-[10.5px] text-[#072116] transition last:border-b-0 hover:bg-[#ddb159]/12 hover:shadow-[inset_3px_0_0_#ddb159]`}
                          >
                            ...
                            <div className="flex min-w-0 items-center gap-2 px-2 font-black">
                              ...
                              <span className="min-w-0 truncate tracking-[-0.01em]">{stock.ticker ?? "—"}</span>
                            </div>
                      ...
                      <div className="px-4 py-8 text-center font-semibold text-[#072116]/60">
                        No ranking data available yet.
                      </div>
                    )}
                  </div>
          ...
          <aside className="grid gap-3 lg:h-full lg:min-h-0 lg:grid-rows-[180px_minmax(0,1fr)_104px] lg:overflow-hidden lg:pb-1">
            <MarketOverviewCard sp500Data={sp500Data} changePct={sp500DailyChangePct} />
            <DailyResearchCard items={whatChangedToday} />
            <PortfolioPromoCard hasPortfolio={Boolean(portfolioSummary)} />
          </aside>
        </div>
      </main>
    </AppShell>
      ...
      <div className="flex min-w-0 items-center gap-2">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-0.5 flex min-w-0 items-center gap-1.5">
            <p className="min-w-0 truncate text-[9px] font-semibold text-[#072116]/45">{stock.company ?? "—"}</p>
            <DailyMovePill changePct={dailyMove} className="h-4 min-w-[38px] px-1 text-[7.5px]" />
          </div>
        </div>
      </div>
...
function StatBlock({ icon, label, main, sub }: { icon: string; label: string; main: string; sub: string }) {
  return (
    <div className="group flex min-h-[74px] items-center gap-3 rounded-xl bg-[#faf6f0] px-3 py-2 text-[#072116] shadow-[0_6px_16px_rgba(0,0,0,0.14)] ring-1 ring-white/30 lg:min-h-0">
      ...
      <div className="min-w-0">
        <p className="truncate text-[8.5px] font-extrabold uppercase tracking-[0.1em] text-[#072116]/55">{label}</p>
        <p className="mt-0.5 truncate text-[15px] font-black leading-tight tracking-[-0.02em] text-[#072116]">{main}</p>
        <p className="truncate text-[9px] font-semibold text-[#072116]/55">{sub}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-w-0 overflow-hidden rounded-2xl border border-[#ddb159]/20 bg-[#faf6f0] p-3 text-[#072116] shadow-[0_12px_30px_rgba(0,0,0,0.18)] lg:min-h-0">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[9px] font-black uppercase tracking-[0.14em] text-[#072116]/55">✦ What changed today?</p>
          <h2 className="mt-1 truncate text-[20px] font-black tracking-[-0.04em]">Daily research brief</h2>
        </div>
        <Link href="/rankings" className="shrink-0 rounded-full border border-[#072116]/10 px-3 py-1.5 text-[10px] font-black text-[#072116]/60 transition hover:border-[#ddb159]/55 hover:text-[#072116]">
          Rankings →
        </Link>
      </div>
          ...
          <Link key={item.ticker} href={`/stock/${item.ticker}`} className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-2 rounded-2xl border border-[#072116]/8 bg-white/72 px-3 py-2 transition hover:border-[#ddb159]/45 hover:bg-[#ddb159]/8">
            <div className="min-w-0">
              <div className="flex min-w-0 items-center gap-2">
                ...
                <span title={item.rankTitle} className={["inline-flex h-5 min-w-[38px] items-center justify-center rounded-full border px-1 text-[8px] font-black", moveClassName(item.rankTone)].join(" ")}>{item.rankLabel}</span>
              </div>
              <p className="mt-0.5 truncate text-[10px] font-semibold text-[#072116]/48">{item.company}</p>
            </div>
            <div className="text-right">
              <p className={["text-[12px] font-black tabular-nums", item.dailyMoveTone === "positive" ? "text-emerald-700" : item.dailyMoveTone === "negative" ? "text-red-700" : "text-[#072116]/55"].join(" ")}>{item.dailyMoveLabel}</p>
              <p className="mt-0.5 text-[9px] font-bold text-[#072116]/42">score {item.score}</p>
            </div>
          </Link>
        ))}
      </div>
  ...
  if (!summary) {
    return (
      <div className="relative min-w-0 overflow-hidden rounded-2xl border border-[#ddb159]/24 bg-[linear-gradient(135deg,#0d3420,#082519_60%,#061b12)] p-4 text-[#faf6f0] shadow-[0_12px_30px_rgba(0,0,0,0.18)]">
        ...
        <h2 className="relative mt-2 text-[24px] font-black leading-none tracking-[-0.05em]">Build your first portfolio.</h2>
        ...
        <Link href="/portfolio?builder=1" className="relative mt-4 inline-flex rounded-full bg-[#ddb159] px-4 py-2 text-[11px] font-black uppercase tracking-[0.1em] text-[#072116] transition hover:brightness-105">
          Start portfolio →
        </Link>
      </div>
    );
  ...
  return (
    <Link href="/portfolio" className="relative min-w-0 overflow-hidden rounded-2xl border border-[#ddb159]/24 bg-[linear-gradient(135deg,#0d3420,#082519_58%,#061b12)] p-4 text-[#faf6f0] shadow-[0_12px_30px_rgba(0,0,0,0.18)] transition hover:border-[#ddb159]/55">
      ...
      <div className="relative flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[9px] font-black uppercase tracking-[0.16em] text-[#ddb159]">✦ Portfolio health</p>
          <h2 className="mt-1 truncate text-[22px] font-black leading-none tracking-[-0.05em]">{summary.name}</h2>
        </div>
        <span className="shrink-0 rounded-full bg-[#ddb159] px-3 py-1 text-[11px] font-black text-[#072116]">{summary.healthScore}/100</span>
      </div>
      <div className="relative mt-4 grid grid-cols-[1fr_auto] items-end gap-3">
        <div>
          <p className="text-[28px] font-black leading-none tracking-[-0.06em]">{money(summary.totalValue, summary.currency)}</p>
          <p className={["mt-1 text-[12px] font-black tabular-nums", isPositive ? "text-emerald-300" : "text-red-200"].join(" ")}>{money(summary.pnl, summary.currency)} · {pct(summary.pnlPct)}</p>
        </div>
        <div className="text-right">
          <p className="text-[10px] font-black uppercase tracking-[0.12em] text-[#faf6f0]/42">Status</p>
          <p className="text-[13px] font-black text-[#ddb159]">{summary.healthLabel}</p>
        </div>
      </div>
      <div className="relative mt-3 grid grid-cols-3 gap-2">
        <MiniMetric label="Holdings" value={String(summary.holdings.length)} />
        <MiniMetric label="Sectors" value={String(summary.sectorCount)} />
        <MiniMetric label="Avg score" value={summary.avgScore ? String(summary.avgScore) : "—"} />
      </div>
      <p className="relative mt-3 truncate text-[11px] font-semibold text-[#faf6f0]/55">
        {topHolding ? `Largest: ${topHolding.ticker} · ${money(topHolding.currentValue, summary.currency)}` : "Add holdings to unlock monitoring."}
      </p>
    </Link>
  );
}
...
function MiniMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-[#ddb159]/14 bg-[#04180f]/58 px-2 py-2">
      <p className="truncate text-[8px] font-black uppercase tracking-[0.1em] text-[#ddb159]/70">{label}</p>
...
function MarketOverviewCard({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[#ddb159]/20 bg-[#faf6f0]/[0.035] p-3 shadow-[0_12px_30px_rgba(0,0,0,0.16)] backdrop-blur lg:min-h-0">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[9px] font-extrabold uppercase tracking-[0.14em] text-[#ddb159]">✦ Market Overview</p>
          <h3 className="mt-0.5 text-[15px] font-black tracking-[-0.02em] text-[#faf6f0]">S&amp;P 500</h3>
        </div>
        <div className="flex flex-col items-end gap-1">
          <p className="rounded-full border border-[#ddb159]/20 bg-[#072116]/50 px-2 py-1 text-[9px] font-black text-[#ddb159]">1D</p>
          {changePct != null && (
            <p className={`rounded-full border px-2 py-0.5 text-[10px] font-black tabular-nums ${changePct >= 0 ? "border-emerald-400/30 bg-emerald-500/10 text-emerald-300" : "border-red-400/30 bg-red-500/10 text-red-300"}`}>
      ...
      <div className="mt-2 hidden h-[96px] overflow-hidden rounded-xl bg-[#072116]/35 lg:block">
        <StockChart ticker="S&P 500" data={sp500Data} initialRange="1D" height={96} compact />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="overflow-hidden rounded-2xl bg-[#faf6f0] p-3 text-[#072116] shadow-[0_10px_26px_rgba(0,0,0,0.2)] lg:flex lg:min-h-0 lg:flex-col">
      <div className="flex shrink-0 items-center justify-between gap-3">
        <p className="text-[9px] font-extrabold uppercase tracking-[0.14em] text-[#072116]/55">✦ Research queue</p>
        <span className="rounded-full border border-[#072116]/10 px-2 py-0.5 text-[8px] font-black text-[#072116]/55">Today</span>
      </div>
      <div className="mt-3 grid gap-2 overflow-hidden lg:min-h-0 lg:flex-1">
        {items.slice(0, 5).map((item) => (
          <Link key={item.ticker} href={`/stock/${item.ticker}`} className="grid grid-cols-[minmax(0,1fr)_auto] gap-2 rounded-xl border border-[#072116]/8 bg-white/70 px-3 py-2 transition hover:border-[#ddb159]/45 hover:bg-[#ddb159]/8">
            <div className="min-w-0">
              <p className="truncate text-[12px] font-black">{item.ticker} · {item.company}</p>
              <p className="mt-0.5 truncate text-[9px] font-bold text-[#072116]/42">{item.sector}</p>
            </div>
            <div className="text-right">
              <p className={["text-[11px] font-black", item.dailyMoveTone === "positive" ? "text-emerald-700" : item.dailyMoveTone === "negative" ? "text-red-700" : "text-[#072116]/55"].join(" ")}>{item.dailyMoveLabel}</p>
              <p className="text-[9px] font-bold text-[#072116]/42">rank {item.rankLabel}</p>
            </div>
          </Link>
        ))}
      </div>
...
function PortfolioPromoCard({ hasPortfolio }: { hasPortfolio: boolean }) {
  return (
    <Link
      href={hasPortfolio ? "/portfolio" : "/portfolio?builder=1"}
      className="group relative flex min-h-[96px] items-center gap-3 overflow-hidden rounded-2xl border border-[#ddb159]/30 bg-[linear-gradient(135deg,#0d3420,#082519_58%,#061f15)] px-3 py-2.5 shadow-[0_14px_34px_rgba(0,0,0,0.22)] transition hover:border-[#ddb159] lg:h-full lg:min-h-0"
    >
      <div className="pointer-events-none absolute -right-8 -top-8 size-20 rounded-full bg-[#ddb159]/20 blur-2xl" />
      <div className="relative flex size-10 shrink-0 items-center justify-center rounded-xl border border-[#ddb159]/30 bg-[#072116]/80 text-[18px] text-[#ddb159]">♛</div>
      <div className="relative flex min-w-0 flex-1 flex-col justify-center">
        <p className="text-[8px] font-black uppercase tracking-[0.14em] text-[#ddb159]">✦ Portfolio tools</p>
        <h2 className="mt-0.5 truncate text-[14px] font-black leading-tight tracking-[-0.03em] text-[#faf6f0]">
          {hasPortfolio ? "Open portfolio command centre" : "Build your AI portfolio"}
        </h2>
        <p className="mt-0.5 truncate text-[10px] font-medium text-[#faf6f0]/65">
          {hasPortfolio ? "Health score, alerts and holdings review." : "Create, import and monitor holdings."}
        </p>
      </div>
      <span className="relative shrink-0 text-[11px] font-bold text-[#ddb159]">Open →</span>
    </Link>
  );
}
