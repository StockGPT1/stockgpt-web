...
function buildPortfolioSummary({
  ...
  const healthScore = Math.round(
    clamp(
      48 + scoreContribution + diversificationScore + (pnlPct > 0 ? 5 : 0) - cashPenalty - weakPenalty - holdingPenalty,
      0,
      100,
    ),
  );
  ...
  return (
    <AppShell activePath="/dashboard">
      <main className="min-h-full overflow-visible lg:h-full lg:min-h-0 lg:overflow-hidden">
        <div className="grid gap-3 lg:h-full lg:min-h-0 lg:grid-cols-[minmax(0,1fr)_330px] lg:gap-3">
          <section className="grid gap-3 lg:h-full lg:min-h-0 lg:grid-rows-[150px_62px_minmax(0,1fr)] lg:overflow-hidden">
          ...
          <aside className="grid gap-3 lg:h-full lg:min-h-0 lg:grid-rows-[168px_188px_minmax(0,1fr)] lg:overflow-hidden lg:pb-1">
              ...
              rankings.map((stock) => {
                ...
                return (
                  <Link
                    ...
                    className={`group grid ${gridClass} h-[10%] min-h-0 items-center border-b border-[#072116]/8 text-[10px] text-[#072116] transition last:border-b-0 hover:bg-[#ddb159]/12 hover:shadow-[inset_3px_0_0_#ddb159]`}
                  >
                    ...
                    <div className="px-2">
                      <span title={move.title} className={["inline-flex h-5 min-w-[38px] items-center justify-center rounded-full border px-1 text-[8px] font-black tabular-nums", moveClassName(move.tone)].join(" ")}>{move.label}</span>
                    </div>
                    ...
                    <div className="flex justify-end px-2">
                      <span className="inline-flex min-w-[58px] justify-center rounded-full bg-[#ddb159] px-2 py-0.5 text-[9px] font-black tabular-nums text-[#072116]">{formatScore(stock.score)}</span>
                    </div>
                  </Link>
                );
              })
  ...
  return (
    <Link href={`/stock/${stock.ticker}`} className="grid min-h-[42px] grid-cols-[32px_minmax(0,1fr)_72px_68px] items-center gap-1 px-3 py-2 text-[11px] text-[#072116] transition hover:bg-[#ddb159]/10">
      ...
      <div className="flex justify-end">
        <span className="inline-flex min-w-[52px] justify-center rounded-full bg-[#ddb159] px-2 py-0.5 text-[9px] font-black tabular-nums text-[#072116]">{formatScore(stock.score)}</span>
      </div>
    </Link>
  );
}
    ...
    <Link href="/portfolio" className="relative min-w-0 overflow-hidden rounded-2xl border border-[#ddb159]/24 bg-[linear-gradient(135deg,#0d3420,#082519_58%,#061b12)] p-4 text-[#faf6f0] shadow-[0_12px_30px_rgba(0,0,0,0.18)] transition hover:border-[#ddb159]/55">
      ...
      <div className="relative flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h2 className="mt-1 truncate text-[20px] font-black leading-none tracking-[-0.05em]">{summary.name}</h2>
        </div>
...
function MarketOverviewCard({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[#ddb159]/20 bg-[#faf6f0]/[0.035] p-3 shadow-[0_12px_30px_rgba(0,0,0,0.16)] backdrop-blur lg:min-h-0">
      ...
      <div className="mt-2 hidden h-[88px] overflow-hidden rounded-xl bg-[#072116]/35 lg:block">
        <StockChart ticker="S&P 500" data={sp500Data} initialRange="1D" height={88} compact />
      </div>
    </div>
  );
}
