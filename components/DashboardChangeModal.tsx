...
function MoverLogo({ ticker, company, compact = false }: { ticker: string; company: string; compact?: boolean }) {
  ...
  return (
    <div className={["grid shrink-0 place-items-center overflow-hidden rounded-full border border-[#ddb159]/18 bg-[#061b12] shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_8px_18px_rgba(0,0,0,0.22)]", compact ? "size-[clamp(38px,4.5vw,52px)] p-2" : "size-12 p-2 sm:size-14 sm:p-2.5"].join(" ")}>
      {symbol && !failed ? (
...
function FilterPill({ label, active = false, loading = false, onClick }: { label: string; active?: boolean; loading?: boolean; onClick: () => void }) {
  const bg = active ? "#ddb159" : "rgba(250,246,240,0.06)";
  const color = active ? "#072116" : "rgba(248,244,232,0.55)";
  return (
    <button type="button" onClick={onClick} style={{ backgroundColor: bg, color }} onMouseEnter={(event) => { event.currentTarget.style.backgroundColor = bg; event.currentTarget.style.color = color; }} onMouseLeave={(event) => { event.currentTarget.style.backgroundColor = bg; event.currentTarget.style.color = color; }} className="stockgpt-top-movers-period-pill inline-flex h-9 shrink-0 items-center justify-center rounded-full border border-[#ddb159]/14 px-4 text-[12px] font-black">
...
function MoverList({ listItems, mode, period, onClose, surface = "default" }: { listItems: DailyChangeItem[]; mode: MoverMode; period: PeriodFilter; onClose: () => void; surface?: "default" | "desktop-drawer" }) {
  if (listItems.length === 0) return <div className="p-6 text-center text-white/50">No mover data available yet.</div>;
  ...
  return (
    <div className={["stockgpt-mover-list overflow-hidden rounded-[22px] border border-[#ddb159]/12", isDesktopDrawer ? "stockgpt-mover-list-desktop bg-[#061b12]" : "bg-[#050706]"].join(" ")} style={{ backgroundColor: isDesktopDrawer ? "#061b12" : "#050706" }}>
      {listItems.map((item) => {
        const label = periodLabel(item, period);
        const tone = periodTone(item, period);
...
export function DashboardChangeModal({ items }: { items: DailyChangeItem[] }) {
  ...
  const listItems = movers.length > 0 ? movers : activeItems;

  return (
    <>
      <section className="grid min-w-0 grid-rows-[auto_auto_minmax(0,1fr)] overflow-hidden rounded-2xl border border-[#ddb159]/20 bg-[#062018] p-3 text-[#f8f4e8] shadow-[0_12px_30px_rgba(0,0,0,0.18)] transition hover:border-[#ddb159]/28 hover:bg-[#062018] sm:p-4 lg:h-full lg:min-h-0 lg:p-[clamp(11px,1vw,15px)]"><button type="button" onClick={() => setOpen(true)} className="min-w-0 shrink-0 text-left"><h2 className="flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[22px] font-black leading-none tracking-[-0.05em] text-[#f8f4e8] sm:text-[26px] lg:text-[clamp(20px,1.72vw,26px)]">Today&apos;s top movers <span className="text-[#ddb159]">›</span></h2><p className="mt-1 text-[9px] font-black uppercase tracking-[0.16em] text-[#ddb159]/78">1D · S&amp;P 500</p></button><div className="mt-3 grid grid-cols-2 rounded-full bg-white/8 p-1">{(["gainers", "losers"] as const).map((tab) => <button key={tab} type="button" onClick={() => setMode(tab)} className={["h-[clamp(34px,4.4dvh,42px)] rounded-full text-[13px] font-black capitalize transition sm:text-[14px] lg:text-[clamp(12px,1vw,14px)]", mode === tab ? "bg-[#ddb159] text-[#072116] shadow-[0_6px_16px_rgba(221,177,89,0.18)]" : "text-[#f8f4e8]/55 hover:bg-white/6 hover:text-[#f8f4e8]"].join(" ")}>Top {tab}</button>)}</div><div className="mt-3 grid min-h-0 grid-cols-4 auto-rows-fr content-stretch gap-x-3 gap-y-3 overflow-hidden pb-1 pt-1">{previewItems.length > 0 ? previewItems.map((item) => <MoverLogoTile key={`${mode}-${period}-${item.ticker}`} item={item} period={period} onOpen={() => setOpen(true)} />) : <button type="button" onClick={() => setOpen(true)} className="col-span-4 h-full rounded-2xl border border-[#ddb159]/12 bg-white/[0.055] px-3 py-4 text-left text-[11px] font-bold text-[#f8f4e8]/45">No {mode === "gainers" ? "positive" : "negative"} movers available for this period yet.</button>}</div></section>
      <MobileSheet open={open} variant="full" eyebrow="Daily market movement" title="Top Movers" description="Best and worst performing stocks from the selected period, with StockGPT rank and score context." onClose={() => setOpen(false)}><div className="stockgpt-top-movers-mobile-content grid min-h-full grid-rows-[auto_auto_minmax(0,1fr)] gap-4 bg-[#050706]" style={{ backgroundColor: "#050706" }}><PeriodPills period={period} loading={loading} onSelect={selectPeriod} /><div className="grid grid-cols-2 rounded-full bg-[#faf6f0]/8 p-1">{(["gainers", "losers"] as const).map((tab) => <button key={tab} type="button" onClick={() => setMode(tab)} className={["h-11 rounded-full text-[14px] font-black capitalize transition", mode === tab ? "bg-[#faf6f0]/14 text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]" : "text-white/50 hover:text-white"].join(" ")}>Top {tab}</button>)}</div><MoverList listItems={listItems} mode={mode} period={period} onClose={() => setOpen(false)} /></div></MobileSheet>
      {open && <div className="stockgpt-top-movers-overlay hidden lg:fixed lg:bottom-0 lg:right-0 lg:top-[92px] lg:z-[210] lg:flex lg:w-full lg:justify-end lg:bg-[#020806]/76 lg:backdrop-blur-sm"><button type="button" onClick={() => setOpen(false)} className="absolute inset-0 cursor-default" aria-label="Close top movers backdrop" /><aside className="stockgpt-top-movers-drawer relative z-10 flex h-full w-[min(540px,42vw)] min-w-[440px] flex-col overflow-hidden rounded-l-[34px] border-l border-[#ddb159]/28 bg-[#050706] text-[#f8f4e8] shadow-[0_28px_90px_rgba(0,0,0,0.58)]" style={{ backgroundColor: "#061b12", color: "#f8f4e8" }}><div className="flex shrink-0 items-start justify-between gap-4 border-b border-[#ddb159]/14 bg-[#050706] px-6 pb-5 pt-6" style={{ backgroundColor: "#04140c" }}><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#ddb159]">Daily market movement</p><h3 className="mt-2 text-[40px] font-black leading-none tracking-[-0.06em]">Top Movers</h3><p className="mt-3 max-w-md text-[14px] font-semibold leading-5 text-[#f8f4e8]/58">Best and worst performing stocks from the selected period, with StockGPT rank and score context.</p></div><button type="button" onClick={() => setOpen(false)} className="grid size-11 shrink-0 place-items-center rounded-full border border-white/10 bg-white/8 text-[22px] font-black text-white transition hover:border-[#ddb159]/50 hover:bg-[#ddb159]/10" aria-label="Close top movers">×</button></div><div className="flex shrink-0 flex-wrap gap-2 bg-[#050706] px-6 py-4" style={{ backgroundColor: "#061b12" }}><PeriodPills period={period} loading={loading} onSelect={selectPeriod} /></div><div className="flex shrink-0 bg-[#050706] px-6 pb-4" style={{ backgroundColor: "#061b12" }}><div className="grid w-full grid-cols-2 rounded-full bg-white/8 p-1">{(["gainers", "losers"] as const).map((tab) => <button key={tab} type="button" onClick={() => setMode(tab)} className={["h-11 rounded-full text-[14px] font-black capitalize transition", mode === tab ? "bg-[#ddb159] text-[#072116] shadow-[0_6px_16px_rgba(221,177,89,0.18)]" : "text-white/50 hover:bg-white/6 hover:text-white"].join(" ")}>Top {tab}</button>)}</div></div><div className="stockgpt-top-movers-scroll min-h-0 flex-1 overflow-y-auto bg-[#050706] px-6 pb-6" style={{ backgroundColor: "#061b12", scrollbarColor: "#d4af37 #061b12" }}><MoverList listItems={listItems} mode={mode} period={period} onClose={() => setOpen(false)} surface="desktop-drawer" /></div></aside></div>}
    </>
  );
}
