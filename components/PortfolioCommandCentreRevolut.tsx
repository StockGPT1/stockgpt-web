...
function holdingUrgencyScore(holding: ExtendedHolding) {
  return (
    holding.actionAlerts.length * 20 +
    holding.eventAlerts.length * 6 +
    (holding.daysSinceReview > 30 ? 8 : 0)
  );
}
...
function preferredInitialRange(data: Partial<Record<TimeRange, ChartPoint[]>>) {
  return (
    RANGE_LABELS.find(({ range }) => hasChartPoints(data, range))?.range ??
    DEFAULT_PORTFOLIO_RANGE
  );
}

function hasChartPoints(
...
function resolveTradeCalculator(
  valueInput: string,
  priceInput: string,
  sharesInput: string,
) {
  return resolveTradeOrder({
    value: valueInput,
    price: priceInput,
    shares: sharesInput,
  });
...
function chartStatusCopy(
  ...
  if (
    hasData &&
    (displayState === "updating" || displayState === "error_with_cache")
  ) {
    return displayState === "error_with_cache"
      ? "Showing cached chart while refreshing"
      : "Refreshing chart";
  }
  ...
  if (displayState === "error_with_cache")
    return "Showing cached chart while refreshing";
  ...
  if (
    hasData &&
    (status === "stale" || status === "sparse" || status === "rebuild_needed")
  ) {
    ...
    <div className="relative h-[260px] overflow-hidden bg-[#072116]/26 px-5 py-6 sm:rounded-xl">
      ...
      <div className="absolute inset-x-4 top-8 grid h-[176px] grid-cols-6 items-end gap-2 opacity-45">
        {[28, 46, 38, 60, 52, 72].map((height, index) => (
          <div
            key={index}
            className="rounded-t-full bg-[#ddb159]/25"
            style={{ height: `${height}%` }}
          />
        ))}
      </div>
      <div className="relative z-10 flex h-full items-center justify-center text-center">
        <div className="max-w-md rounded-3xl border border-[#ddb159]/16 bg-[#04140c]/80 px-5 py-4 shadow-[0_18px_48px_rgba(0,0,0,0.20)] backdrop-blur">
          <p className="text-[15px] font-black text-[#ddb159]">
            {chartStateTitle(meta)}
          </p>
          <p className="mt-2 text-[12px] font-semibold leading-5 text-[#faf6f0]/58">
            {chartStateDetail(meta)}
          </p>
        </div>
      </div>
    </div>
  ...
  if (section === "overview")
    return (
      <svg viewBox="0 0 24 24" className="size-5" aria-hidden="true">
        <path {...common} d="M4 13.5 9.5 8l4 4L20 5.5" />
  ...
  if (section === "activity")
    return (
      <svg viewBox="0 0 24 24" className="size-5" aria-hidden="true">
        ...
        <path {...common} d="M8 17v-5M13 17V7M18 17v-8" />
      </svg>
    );
  return (
    <svg viewBox="0 0 24 24" className="size-5" aria-hidden="true">
      <path
        {...common}
        d="M12 3v3M12 18v3M4.8 6.8l2.1 2.1M17.1 17.1l2.1 2.1M3 12h3M18 12h3M4.8 17.2l2.1-2.1M17.1 6.9l2.1-2.1"
      />
    ...
    <button
      ...
      className={[
        ...
        active === section
          ...
          : "text-[#faf6f0]/56 hover:bg-[#ddb159]/10 hover:text-[#faf6f0]",
      ].join(" ")}
    >
      {label}
    </button>
...
function PortfolioTopBar({
  ...
  const portfolioOptions = portfolios.map((portfolio) => ({
    ...
    description: portfolio.createdAt
  ...
  return (
    <div className="hidden min-w-0 flex-col gap-2 rounded-2xl border border-white/8 bg-black/18 p-2.5 backdrop-blur sm:flex sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0 px-1">
        ...
        <p className="mt-0.5 truncate text-[13px] font-black text-[#faf6f0]">
        ...
        {portfolios.length > 1 && (
          <StockGPTSelect
            ...
            onChange={(nextPortfolioId) =>
              router.push(`/portfolio?portfolio=${nextPortfolioId}`)
            }
  ...
  const activeRangeSignature = useMemo(
    () =>
      activeRangePoints
        .map(
          (point) =>
            `${point.date}:${point.close}:${point.basis ?? ""}:${point.pnl ?? ""}:${point.pnlPct ?? ""}`,
        )
        .join(";"),
    [activeRangePoints],
  );
    ...
    <section className="portfolio-chart-hero relative overflow-hidden rounded-[28px] border border-[#ddb159]/18 bg-[radial-gradient(circle_at_50%_8%,rgba(221,177,89,0.13),transparent_34%),linear-gradient(180deg,#092116_0%,#04140c_56%,#020805_100%)] text-[#faf6f0] shadow-[0_18px_48px_rgba(0,0,0,0.30)] sm:rounded-[32px]">
          ...
          <div className="min-w-0 flex-1">
            {portfolios.length > 1 ? (
              <>
                ...
                />
                <p className="hidden truncate text-[10px] font-black uppercase tracking-[0.16em] text-[#ddb159] sm:block">
                  {portfolioName}
                </p>
              </>
            ) : (
        ...
        <div className="mt-4 flex flex-col items-center gap-3 sm:flex-row sm:justify-between">
          ...
          <div className="flex shrink-0 items-center gap-1 rounded-full bg-white/[0.07] p-1">
            {availableRanges.length > 0 ? (
              ...
              <span className="px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.1em] text-[#faf6f0]/52">
                Building
              </span>
            )}
          </div>
      ...
      <div className="relative hidden grid-cols-3 gap-px border-t border-white/8 bg-white/5 text-center sm:grid sm:text-left">
        <div className="px-4 py-3">
          <p className="text-[9px] font-black uppercase tracking-[0.12em] text-[#faf6f0]/38">
            Open positions
          </p>
        ...
        <div className="px-4 py-3">
          ...
          <p className="mt-1 text-[18px] font-black text-[#ddb159]">
            {summary.weightedAvgScore ?? "—"}
          </p>
        </div>
        <div className="px-4 py-3">
          <p className="text-[9px] font-black uppercase tracking-[0.12em] text-[#faf6f0]/38">
          ...
          <p className="mt-0.5 text-[10px] font-bold text-[#faf6f0]/42">
            {summary.cashDrag.toFixed(1)}% drag
          </p>
        </div>
      </div>
    </section>
...
function AddCashPanel({
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = await addCash({ portfolioId, amount: value / inputRate });
      if (!result.success) {
  ...
  return (
    <div className="rounded-2xl bg-[#faf6f0] p-4 text-[#072116]">
      ...
      {message && (
        <p className="mt-2 rounded-xl bg-white px-3 py-2 text-[11px] font-bold text-[#072116]/65">
          {message}
        </p>
      )}
    </div>
...
function MiniMetric({
  ...
  return (
    <div className="min-w-0 rounded-xl border border-[#ddb159]/12 bg-[#faf6f0]/[0.045] px-2.5 py-2">
      <p className="truncate text-[8px] font-black uppercase tracking-[0.1em] text-[#faf6f0]/40">
        {label}
      </p>
      <p
        className={`mt-1 truncate text-[13px] font-black leading-none ${valueClass}`}
      >
        {value}
      </p>
      <p className="mt-1 truncate text-[9px] font-semibold text-[#faf6f0]/42">
        {sub}
      </p>
    </div>
  );
...
function ManageHoldingModal({
  ...
  const router = useRouter();
  const [tradeMode, setTradeMode] = useState<"trim" | "buy">("trim");
  const [trimValue, setTrimValue] = useState("");
  const [trimPrice, setTrimPrice] = useState(
    holding.currentPrice > 0
      ? String(Number(holding.currentPrice.toFixed(2)))
      : "",
  );
  ...
  const trimCalculation = useMemo(
    () => resolveTradeCalculator(trimValue, trimPrice, trimShares),
    [trimValue, trimPrice, trimShares],
  );
  const buyCalculation = useMemo(
    () => resolveTradeCalculator(buyValue, buyPrice, buyShares),
    [buyValue, buyPrice, buyShares],
  );
  ...
  function prefillSuggestedTrim() {
    const pctToUse =
      action.suggestedTrimRange?.[0] ?? recommendation.pct ?? null;
    if (pctToUse == null || holding.currentPrice <= 0) return;
    const shares = holding.shares * (pctToUse / 100);
    ...
    setTrimValue(String(Number(value.toFixed(2))));
  }
  ...
  function runTrim() {
    if (
      ...
      setMessage(trimCalculation.error ?? "Enter a valid trim order.");
      return;
    }

    if (trimCalculation.shares > holding.shares + 0.000001) {
      setMessage("You cannot trim more shares than you hold.");
      return;
    }

    setMessage("Trimming holding...");
    startTransition(async () => {
      const result = await trimHolding({
        portfolioId,
        ticker: holding.ticker,
        value: trimCalculation.value,
        price: trimCalculation.price,
        shares: trimCalculation.shares,
      });
      if (!result.success) {
        setMessage(result.error ?? "Could not update holding.");
        return;
      }
      setMessage(`${holding.ticker} trimmed. Proceeds were added to cash.`);
      window.setTimeout(() => router.refresh(), 120);
      ...
      const result =
        mode === "cash"
          ? await buyHoldingWithCash({
              portfolioId,
              ticker: holding.ticker,
              value: buyCalculation.value ?? 0,
              price: buyCalculation.price ?? 0,
  ...
  const trimPreview =
    trimCalculation.error == null &&
    trimCalculation.value != null &&
    trimCalculation.price != null &&
    trimCalculation.shares != null
      ? `${num(trimCalculation.shares, 6)} shares at ${money(trimCalculation.price)} for ${money(trimCalculation.value)}`
      : trimCalculation.error;
  const buyPreview =
    buyCalculation.error == null &&
    buyCalculation.value != null &&
    buyCalculation.price != null &&
    buyCalculation.shares != null
      ...
      : buyCalculation.error;
  const scoreMoved =
    holding.scoreAtEntry != null || holding.rankAtEntry != null;
  const actionUpdatedLabel = formatFreshness(
    action.dataUpdatedAt ?? action.generatedAt,
  );
  ...
  return createPortal(
    <div className="fixed inset-0 z-[2147483647] flex items-stretch justify-center bg-[#020805]/88 p-3 pt-[calc(0.75rem+env(safe-area-inset-top))] text-[#faf6f0] backdrop-blur-md sm:items-center sm:p-4 lg:justify-end">
      <button
        type="button"
        aria-label="Close manage holding"
        onClick={onClose}
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label={`Manage ${holding.ticker}`}
        className="stockgpt-manage-holding-dialog relative z-10 flex max-h-[calc(100dvh-1.5rem)] w-full max-w-xl flex-col overflow-hidden rounded-[28px] border border-[#ddb159]/24 bg-[#061b12] text-[#faf6f0] shadow-[0_24px_90px_rgba(0,0,0,0.62)] sm:max-h-[calc(100dvh-2rem)]"
      >
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-[#ddb159]/14 bg-[#04140c] p-4 sm:p-5">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <h3 className="truncate text-[28px] font-black leading-none tracking-[-0.05em]">
                  {holding.ticker}
                </h3>
                <p className="mt-1 truncate text-[12px] font-bold text-[#faf6f0]/52">
          ...
          <button
            type="button"
            onClick={onClose}
            className="grid size-10 shrink-0 place-items-center rounded-full border border-[#ddb159]/18 bg-[#faf6f0]/[0.045] text-xl text-[#ddb159] transition hover:bg-[#ddb159]/10"
          >
            &times;
          </button>
        </div>
        <div className="grid min-h-0 gap-3 overflow-y-auto p-4 sm:p-5">
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            ...
            <MiniMetric
              label="Avg entry"
              value={money(holding.entryPrice)}
              sub="cost basis"
            />
            <MiniMetric
              label="Allocation"
            ...
            <MiniMetric
              label="P/L"
              value={money(holding.totalPnLDollars)}
              sub={pct(holding.pnlPercent)}
              tone={holding.totalPnLDollars >= 0 ? "positive" : "negative"}
            />
            <MiniMetric
              label="Score"
              value={num(holding.score, 0)}
              sub={`rank #${holding.rank ?? "-"}`}
              tone="gold"
            />
            <MiniMetric
              label="Cash"
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <div className="rounded-2xl border border-[#ddb159]/14 bg-[#faf6f0]/[0.045] p-3">
              <p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#faf6f0]/42">
                Recent movement
              </p>
              <p className="mt-1 text-[13px] font-black text-[#faf6f0]">
                Recent price movement unavailable
              </p>
              <p className="mt-1 text-[11px] font-semibold leading-4 text-[#faf6f0]/50">
                StockGPT does not have enough reliable price history in this
                modal for 24h, 5D, or 1M labels yet.
              </p>
            </div>
            <div className="rounded-2xl border border-[#ddb159]/14 bg-[#faf6f0]/[0.045] p-3">
              <p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#faf6f0]/42">
              ...
              {scoreMoved ? (
                <>
                  <p className="mt-1 text-[13px] font-black text-[#faf6f0]">
                    Score{" "}
                    {holding.scoreAtEntry == null
                      ? "—"
                      : num(holding.scoreAtEntry, 0)}{" "}
                    → {num(holding.score, 0)} since entry
                  </p>
                  <p className="mt-1 text-[11px] font-semibold leading-4 text-[#faf6f0]/50">
                    Rank{" "}
                    {holding.rankAtEntry == null
                      ? "—"
                      : `#${holding.rankAtEntry}`}{" "}
                    → {holding.rank == null ? "—" : `#${holding.rank}`}
                  </p>
                </>
              ) : (
                <p className="mt-1 text-[11px] font-semibold leading-4 text-[#faf6f0]/50">
                  AI score movement unavailable for this timeframe.
                </p>
              )}
            </div>
          </div>
          <div className="rounded-2xl border border-[#ddb159]/20 bg-[#ddb159]/10 p-3">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-[18px] font-black tracking-[-0.03em] text-[#faf6f0]">
                  {action.label}
                </p>
              </div>
              <span className="rounded-full border border-[#ddb159]/18 px-2 py-1 text-[9px] font-black uppercase tracking-[0.1em] text-[#ddb159]">
                {actionUpdatedLabel}
              </span>
            </div>
            <p className="mt-2 text-[13px] font-semibold leading-5 text-[#faf6f0]/68">
              {action.plainEnglishReason}
            </p>
            {action.freshness === "stale" && (
              <p className="mt-2 rounded-2xl border border-amber-300/20 bg-amber-300/10 px-3 py-2 text-[11px] font-bold text-amber-100">
                Data is stale, so StockGPT blocks high-confidence action
                suggestions until rankings/diagnostics refresh.
              </p>
            )}
            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              {action.evidence.slice(0, 4).map((item) => (
                <p
                  key={item}
                  className="rounded-xl bg-[#061b12]/72 px-3 py-2 text-[10px] font-bold leading-4 text-[#faf6f0]/58"
                >
                  {item}
                </p>
              ))}
            </div>
            {reinvestment && action.action === "trim" && (
              <p className="mt-3 rounded-2xl border border-[#ddb159]/16 bg-[#04140c]/80 p-3 text-[11px] font-semibold leading-4 text-[#faf6f0]/55">
                If you later reallocate trimmed cash, compare against{" "}
                {reinvestment.ticker}. This is a research alternative, not an
                instruction to reinvest immediately.
              </p>
            )}
          </div>
          <div className="grid gap-2 sm:grid-cols-2">
            <button
              type="button"
              onClick={() => setTradeMode("trim")}
              className={[
                "h-11 rounded-2xl border px-4 text-[11px] font-black uppercase tracking-[0.1em] transition",
                tradeMode === "trim"
                  ? "border-[#ddb159] bg-[#ddb159] text-[#061b12]"
                  : "border-[#ddb159]/22 text-[#ddb159] hover:border-[#ddb159]/45 hover:bg-[#ddb159]/10",
              ].join(" ")}
            >
              Trim
            </button>
          ...
          {tradeMode === "trim" ? (
            <div className="rounded-2xl border border-[#ddb159]/14 bg-[#faf6f0]/[0.045] p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-[11px] font-semibold leading-4 text-[#faf6f0]/50">
                    Enter any two fields. Use the actual order price for
                    accurate proceeds and realised P/L.
                  </p>
                </div>
                {(action.suggestedTrimRange || recommendation.pct != null) && (
                  <button
                    type="button"
                    onClick={prefillSuggestedTrim}
                    className="rounded-full border border-[#ddb159]/22 px-3 py-1.5 text-[9px] font-black uppercase tracking-[0.08em] text-[#ddb159] hover:border-[#ddb159]/45 hover:bg-[#ddb159]/10"
                  >
                    Prefill suggestion
                  </button>
                )}
              ...
              <div className="mt-2 grid gap-2 sm:grid-cols-3">
                <label className="min-w-0">
                  <span className="mb-1 block text-[8px] font-black uppercase tracking-[0.12em] text-[#faf6f0]/38">
                    Value sold
                  </span>
                  <input
                    type="number"
                    ...
                    step="0.01"
                    value={trimValue}
                    onChange={(event) => setTrimValue(event.target.value)}
                    placeholder="500"
                    className="h-11 w-full rounded-2xl border border-[#ddb159]/18 bg-[#020805]/55 px-3 text-[13px] font-black text-[#faf6f0] outline-none placeholder:text-[#faf6f0]/32 focus:border-[#ddb159]"
                  />
                </label>
                <label className="min-w-0">
                  <span className="mb-1 block text-[8px] font-black uppercase tracking-[0.12em] text-[#faf6f0]/38">
                  ...
                  <input
                    ...
                    step="0.01"
                    value={trimPrice}
                    onChange={(event) => setTrimPrice(event.target.value)}
                    placeholder="0.00"
                    className="h-11 w-full rounded-2xl border border-[#ddb159]/18 bg-[#020805]/55 px-3 text-[13px] font-black text-[#faf6f0] outline-none placeholder:text-[#faf6f0]/32 focus:border-[#ddb159]"
                  />
                </label>
                <label className="min-w-0">
                  ...
                  <input
                    ...
                    step="0.000001"
                    value={trimShares}
                    onChange={(event) => setTrimShares(event.target.value)}
                    placeholder="0"
                    className="h-11 w-full rounded-2xl border border-[#ddb159]/18 bg-[#020805]/55 px-3 text-[13px] font-black text-[#faf6f0] outline-none placeholder:text-[#faf6f0]/32 focus:border-[#ddb159]"
                  />
                </label>
              ...
              <p
                className={[
                  "mt-2 rounded-2xl px-3 py-2 text-[11px] font-bold",
                  trimCalculation.error
                    ? "bg-red-500/10 text-red-200"
                    : "bg-[#061b12]/72 text-[#faf6f0]/58",
                ].join(" ")}
              >
                {trimPreview}
              </p>
              <button
                type="button"
                disabled={isPending || Boolean(trimCalculation.error)}
                onClick={runTrim}
                className="mt-2 h-11 w-full rounded-2xl bg-[linear-gradient(180deg,#f3d98b,#d6ae4d_55%,#a77d2e)] px-5 text-[11px] font-black uppercase tracking-[0.1em] text-[#061b12] disabled:opacity-50"
              >
                Trim
              </button>
            </div>
            ...
            <div className="rounded-2xl border border-[#ddb159]/14 bg-[#faf6f0]/[0.045] p-3">
              ...
              <p className="mt-1 text-[11px] font-semibold leading-4 text-[#faf6f0]/50">
                Enter any two fields. Cash mode reduces portfolio cash; external
                mode adds to the holding without using cash.
              </p>
              <div className="mt-2 grid gap-2 sm:grid-cols-3">
                <label className="min-w-0">
                  <span className="mb-1 block text-[8px] font-black uppercase tracking-[0.12em] text-[#faf6f0]/38">
                  ...
                  <input
                    ...
                    placeholder="500"
                    className="h-10 w-full rounded-2xl border border-[#ddb159]/18 bg-[#020805]/55 px-3 text-[13px] font-black text-[#faf6f0] outline-none placeholder:text-[#faf6f0]/32 focus:border-[#ddb159]"
                  />
                </label>
                <label className="min-w-0">
                  <span className="mb-1 block text-[8px] font-black uppercase tracking-[0.12em] text-[#faf6f0]/38">
              ...
              <div className="mt-2 grid gap-2 sm:grid-cols-2">
                <button
                  type="button"
                  disabled={isPending || Boolean(buyCalculation.error)}
                  onClick={() => runBuyMore("cash")}
                  className="h-10 rounded-2xl border border-[#ddb159]/30 px-4 text-[10px] font-black uppercase tracking-[0.1em] text-[#ddb159] disabled:opacity-50"
                >
                  Buy more with cash
                </button>
                <button
                  type="button"
                  disabled={isPending || Boolean(buyCalculation.error)}
                  onClick={() => runBuyMore("external")}
                  className="h-10 rounded-2xl border border-[#ddb159]/16 px-4 text-[10px] font-black uppercase tracking-[0.1em] text-[#faf6f0]/62 disabled:opacity-50"
                >
          ...
          {message && (
            <p className="rounded-2xl bg-[#faf6f0]/[0.055] px-3 py-2 text-[11px] font-bold text-[#faf6f0]/62">
              {message}
            </p>
          )}
        </div>
...
function HoldingCell({
  ...
  return (
    <div className="min-w-0 lg:text-right">
      <p className="text-[8px] font-black uppercase tracking-[0.1em] text-[#072116]/36 lg:hidden">
        {label}
      </p>
      <p
        className={`truncate text-[13px] font-black tabular-nums ${valueClass}`}
      >
        {value}
      </p>
      {sub && (
        <p className="mt-0.5 truncate text-[10px] font-bold tabular-nums text-[#072116]/50">
          {sub}
        </p>
      )}
    </div>
      ...
      {holdings.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-[#ddb159]/24 bg-[#061b12]/72 p-6 text-center text-[#faf6f0]">
          ...
          <p className="mx-auto mt-2 max-w-xl text-[13px] font-semibold leading-6 text-[#faf6f0]/52">
            Use Add / Import to add cash, log holdings or import from Trading
            212.
          </p>
        </div>
      ) : (
        <div className="grid gap-2">
          {!preview && (
            <div className="hidden grid-cols-[minmax(190px,1.35fr)_minmax(96px,0.75fr)_minmax(108px,0.85fr)_minmax(76px,0.55fr)_minmax(92px,0.7fr)_minmax(112px,auto)] px-4 text-[9px] font-black uppercase tracking-[0.12em] text-[#faf6f0]/38 lg:grid">
              <span>Asset</span>
...
function ManagePanel({
  ...
  function runDelete() {
    if (
      !window.confirm(
        `Delete "${portfolioName}"? This removes this portfolio and its holdings only.`,
      )
    )
      return;
    startDeleting(async () => {
  ...
  const objectiveOptions = [
    {
      value: "growth",
      label: "Growth",
      description: "Prioritise higher upside names.",
    },
    {
      value: "income",
      label: "Income",
      description: "Prefer steadier cash-generating holdings.",
    },
    {
      value: "balanced",
      label: "Balanced",
      description: "Blend growth and risk control.",
    },
    {
      value: "capital_preservation",
      label: "Preservation",
      description: "Reduce concentration and drawdown risk.",
    },
    {
      value: "watchlist",
      label: "Watchlist",
      description: "Track ideas without aggressive scoring.",
    },
  ];
  const riskOptions = [
    {
      value: "conservative",
      label: "Conservative",
      description: "Lower volatility and concentration.",
    },
    {
      value: "moderate",
      label: "Moderate",
    ...
    },
    {
      value: "aggressive",
      label: "Aggressive",
      description: "Higher tolerance for volatility.",
    },
  ];
  ...
  return (
    <div className="grid gap-3">
      <div className="rounded-2xl bg-[#faf6f0] p-4 text-[#072116]">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto_auto]">
          ...
          <button
            ...
            onClick={runDelete}
            disabled={isDeleting}
            className="h-11 rounded-2xl border border-red-500/35 px-5 text-[11px] font-black uppercase tracking-[0.1em] text-red-700 disabled:opacity-60"
          >
      ...
      <div className="rounded-2xl bg-[#faf6f0] p-4 text-[#072116]">
        ...
        <h3 className="mt-1 text-[20px] font-black tracking-[-0.04em]">
          Objective, risk and horizon
        </h3>
        <p className="mt-1 text-[12px] font-semibold leading-5 text-[#072116]/50">
          These settings guide portfolio health scoring, alerts and trim
          recommendations.
        </p>
        <div className="mt-3 grid gap-2 lg:grid-cols-3">
          <StockGPTSelect
            ...
            ariaLabel="Portfolio objective"
            buttonClassName="h-11 rounded-2xl bg-white text-[#072116]"
          />
          <StockGPTSelect
            value={riskValue}
            options={riskOptions}
        ...
        <button
          ...
          disabled={isSavingPrefs || preferencesUnchanged}
          className="mt-3 h-11 rounded-2xl bg-[#072116] px-5 text-[11px] font-black uppercase tracking-[0.1em] text-[#ddb159] disabled:opacity-60"
        >
          {isSavingPrefs ? "Saving..." : "Save preferences"}
        </button>
      </div>
      {message && (
        <p className="rounded-xl bg-[#faf6f0] px-3 py-2 text-[11px] font-bold text-[#072116]/65">
          {message}
        </p>
      )}
    </div>
          ...
          <div className="flex flex-wrap gap-2 sm:justify-end">
            {transaction.shares != null && (
              <span className="rounded-full bg-[#072116]/6 px-2 py-1 text-[10px] font-bold text-[#072116]/60">
                {num(transaction.shares, 4)} shares
              </span>
            )}
...
export function PortfolioCommandCentreRevolut({
  ...
  return (
    <div className="grid min-w-0 max-w-full gap-3 overflow-x-hidden">
      ...
      {section === "activity" && (
        <ActivityPanel transactions={transactions} currency={currency} />
      )}
      {section === "manage" && (
        <ManagePanel
          portfolioId={portfolioId}
