...
function AddCashPanel({ portfolioId, currency }: { portfolioId: string; currency: string }) {
  ...
  function submit() {
    ...
    setMessage("Adding cashâ€¦");
    startTransition(async () => { const result = await addCash({ portfolioId, amount: value }); if (!result.success) { setMessage(result.error ?? "Could not add cash."); return; } setAmount(""); setMessage("Cash added."); window.setTimeout(() => router.refresh(), 120); });
  }
...
function AddHoldingPanel({ portfolioId, stockOptions }: { portfolioId: string; stockOptions: StockOption[] }) {
  ...
  function submit() {
    ...
    setMessage("Adding holdingâ€¦");
    startTransition(async () => { const result = await logExistingHolding({ portfolioId, ticker: exactMatch.ticker, shares: shareCount, entryPrice: price }); if (!result.success) { setMessage(result.error ?? "Could not add holding."); return; } setQuery(""); setShares(""); setEntryPrice(""); setMessage(`${exactMatch.ticker} added.`); window.setTimeout(() => router.refresh(), 120); });
  }
...
function ManageHoldingModal({ portfolioId, holding, recommendation, onClose }: { portfolioId: string; holding: ExtendedHolding; recommendation: PortfolioTrimRecommendation; onClose: () => void }) {
  ...
  function runPlan(rawPercent: number) { const percent = Math.round(Number(rawPercent) * 10) / 10; if (!Number.isFinite(percent) || percent <= 0 || percent > 100) { setMessage("Enter a percentage between 1 and 100."); return; } setMessage("Updating holdingâ€¦"); startTransition(async () => { const result = await trimHolding({ portfolioId, ticker: holding.ticker, percentage: percent }); if (!result.success) { setMessage(result.error ?? "Could not update holding."); return; } setMessage(`${holding.ticker} adjusted by ${percent}%. Cash has been added for the next allocation step.`); window.setTimeout(() => router.refresh(), 120); window.setTimeout(onClose, 650); }); }
  function runRemove(creditCash: boolean) { const label = creditCash ? "close this position and credit cash" : "remove this holding without adding cash"; if (!window.confirm(`Are you sure you want to ${label} for ${holding.ticker}?`)) return; setMessage("Removing holdingâ€¦"); startTransition(async () => { const result = creditCash ? await trimHolding({ portfolioId, ticker: holding.ticker, percentage: 100 }) : await removeHolding({ portfolioId, ticker: holding.ticker, creditCash: false }); if (!result.success) { setMessage(result.error ?? "Could not update holding."); return; } setMessage(creditCash ? `${holding.ticker} closed.` : `${holding.ticker} removed.`); window.setTimeout(() => router.refresh(), 120); window.setTimeout(onClose, 650); }); }
...
function ManagePanel({ portfolioId, portfolioName }: { portfolioId: string; portfolioName: string }) {
  ...
  function saveName() { const cleanName = name.trim(); if (!cleanName) { setMessage("Portfolio name cannot be empty."); return; } setMessage("Saving nameâ€¦"); startRenaming(async () => { const result = await renamePortfolio({ portfolioId, name: cleanName }); if (!result.success) { setMessage(result.error ?? "Could not rename portfolio."); return; } setMessage("Portfolio renamed."); window.setTimeout(() => router.refresh(), 120); }); }
