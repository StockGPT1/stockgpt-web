...
function PortfolioChartHero({ portfolioId, portfolios, portfolioName, currency, summary, chartData, chartMeta, createdAt, cashBalance }: { portfolioId: string; portfolios: PortfolioOption[]; portfolioName: string; currency: string; summary: ReturnType<typeof buildPortfolioHealthSummary>; chartData: Partial<Record<TimeRange, ChartPoint[]>>; chartMeta?: PortfolioChartMeta; createdAt?: string | null; cashBalance: number }) {
  ...
  return (
    <section className="portfolio-chart-hero relative overflow-hidden rounded-[28px] border border-[#ddb159]/18 bg-[radial-gradient(circle_at_50%_8%,rgba(221,177,89,0.13),transparent_34%),linear-gradient(180deg,#092116_0%,#04140c_56%,#020805_100%)] text-[#faf6f0] shadow-[0_18px_48px_rgba(0,0,0,0.30)] sm:rounded-[32px]">
      ...
      <div className="relative -mx-5 sm:mx-0">{activeRangeHasData ? <StockChart key={`${portfolioId}-${activeRange}-ready`} ticker="Portfolio" data={chartRangeData} initialRange={activeRange} height={260} compact color="#ddb159" mobileTransparentFrame onScrub={handleChartScrub} /> : <div className="flex h-[260px] items-center justify-center bg-[#072116]/26 px-6 text-center sm:rounded-xl"><div><p className="text-[15px] font-black text-[#ddb159]">{chartStatus ?? "Building chart history..."}</p><p className="mx-auto mt-2 max-w-md text-[12px] font-semibold leading-5 text-[#faf6f0]/50">StockGPT is repairing this portfolio snapshot history. The chart will appear once enough real portfolio snapshots are available.</p></div></div>}</div>
...
function AddCashPanel({ portfolioId, currency }: { portfolioId: string; currency: string }) {
  ...
  function submit() {
    ...
    startTransition(async () => { const result = await addCash({ portfolioId, amount: value }); if (!result.success) { setMessage(result.error ?? "Could not add cash."); return; } setAmount(""); setMessage("Cash added."); router.refresh(); });
  }
...
function AddHoldingPanel({ portfolioId, stockOptions }: { portfolioId: string; stockOptions: StockOption[] }) {
  ...
  function submit() {
    ...
    startTransition(async () => { const result = await logExistingHolding({ portfolioId, ticker: exactMatch.ticker, shares: shareCount, entryPrice: price }); if (!result.success) { setMessage(result.error ?? "Could not add holding."); return; } setQuery(""); setShares(""); setEntryPrice(""); setMessage(`${exactMatch.ticker} added.`); router.refresh(); });
  }
...
function ManageHoldingModal({ portfolioId, holding, recommendation, onClose }: { portfolioId: string; holding: ExtendedHolding; recommendation: PortfolioTrimRecommendation; onClose: () => void }) {
  ...
  const [message, setMessage] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();
  useEffect(() => { function onKeyDown(event: KeyboardEvent) { if (event.key === "Escape") onClose(); } const originalOverflow = document.body.style.overflow; const originalOverscroll = document.body.style.overscrollBehavior; document.body.style.overflow = "hidden"; document.body.style.overscrollBehavior = "none"; document.addEventListener("keydown", onKeyDown); return () => { document.body.style.overflow = originalOverflow; document.body.style.overscrollBehavior = originalOverscroll; document.removeEventListener("keydown", onKeyDown); }; }, [onClose]);
  function runPlan(rawPercent: number) { const percent = Math.round(Number(rawPercent) * 10) / 10; if (!Number.isFinite(percent) || percent <= 0 || percent > 100) { setMessage("Enter a percentage between 1 and 100."); return; } startTransition(async () => { const result = await trimHolding({ portfolioId, ticker: holding.ticker, percentage: percent }); if (!result.success) { setMessage(result.error ?? "Could not update holding."); return; } setMessage(`${holding.ticker} adjusted by ${percent}%. Cash has been added for the next allocation step.`); router.refresh(); window.setTimeout(onClose, 650); }); }
  function runRemove(creditCash: boolean) { const label = creditCash ? "close this position and credit cash" : "remove this holding without adding cash"; if (!window.confirm(`Are you sure you want to ${label} for ${holding.ticker}?`)) return; startTransition(async () => { const result = creditCash ? await trimHolding({ portfolioId, ticker: holding.ticker, percentage: 100 }) : await removeHolding({ portfolioId, ticker: holding.ticker, creditCash: false }); if (!result.success) { setMessage(result.error ?? "Could not update holding."); return; } setMessage(creditCash ? `${holding.ticker} closed.` : `${holding.ticker} removed.`); router.refresh(); window.setTimeout(onClose, 650); }); }
  const reinvestment = recommendation.reinvestment;
  return createPortal(<div className="fixed inset-0 z-[2147483647] flex items-stretch justify-center bg-[#020805]/88 p-3 pt-[calc(0.75rem+env(safe-area-inset-top))] text-[#faf6f0] backdrop-blur-md sm:items-center sm:p-4 lg:justify-end"><button type="button" aria-label="Close manage holding" onClick={onClose} className="absolute inset-0 cursor-default" /><div role="dialog" aria-modal="true" aria-label={`Manage ${holding.ticker}`} className="stockgpt-manage-holding-dialog relative z-10 flex max-h-[calc(100dvh-1.5rem)] w-full max-w-xl flex-col overflow-hidden rounded-[28px] border border-[#ddb159]/24 bg-[#061b12] text-[#faf6f0] shadow-[0_24px_90px_rgba(0,0,0,0.62)] sm:max-h-[calc(100dvh-2rem)]"><div className="flex shrink-0 items-start justify-between gap-3 border-b border-[#ddb159]/14 bg-[#04140c] p-4 sm:p-5"><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#ddb159]">Manage holding</p><div className="mt-2 flex min-w-0 items-center gap-3"><StockLogo ticker={holding.ticker} size={38} /><div className="min-w-0"><h3 className="truncate text-[28px] font-black leading-none tracking-[-0.05em]">{holding.ticker}</h3><p className="mt-1 truncate text-[12px] font-bold text-[#faf6f0]/52">{holding.company ?? "Holding"}</p></div></div></div><button type="button" onClick={onClose} className="grid size-10 shrink-0 place-items-center rounded-full border border-[#ddb159]/18 bg-[#faf6f0]/[0.045] text-xl text-[#ddb159] transition hover:bg-[#ddb159]/10">&times;</button></div><div className="grid min-h-0 gap-3 overflow-y-auto p-4 sm:p-5"><div className="grid grid-cols-2 gap-2 sm:grid-cols-4"><MiniMetric label="Value" value={money(holding.currentValue)} sub={`${num(holding.shares, 4)} sh`} /><MiniMetric label="Allocation" value={`${holding.currentAllocationPct.toFixed(1)}%`} sub={`target ${holding.targetAllocationPct?.toFixed(1) ?? "—"}%`} /><MiniMetric label="P/L" value={money(holding.totalPnLDollars)} sub={pct(holding.pnlPercent)} tone={holding.totalPnLDollars >= 0 ? "positive" : "negative"} /><MiniMetric label="Score" value={num(holding.score, 0)} sub={`rank #${holding.rank ?? "—"}`} tone="gold" /></div><div className="rounded-2xl border border-[#ddb159]/20 bg-[#ddb159]/10 p-3"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#ddb159]">Recommended action</p><p className="mt-1 text-[18px] font-black tracking-[-0.03em] text-[#faf6f0]">{recommendation.label}</p><p className="mt-2 text-[13px] font-semibold leading-5 text-[#faf6f0]/68">{recommendation.reason}</p>{reinvestment && <div className="mt-3 rounded-2xl border border-[#ddb159]/16 bg-[#04140c]/80 p-3"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-[#ddb159]">Reinvestment candidate {reinvestment.sameSector ? "· Same sector" : "· Cross-sector fallback"}</p><div className="mt-2 flex min-w-0 items-center gap-3"><StockLogo ticker={reinvestment.ticker} size={34} /><div className="min-w-0 flex-1"><p className="truncate text-[16px] font-black text-[#faf6f0]">{reinvestment.ticker} <span className="text-[#faf6f0]/45">· {reinvestment.company ?? "StockGPT ranked stock"}</span></p><p className="mt-0.5 truncate text-[11px] font-bold text-[#faf6f0]/50">{reinvestment.sector ?? "Unclassified sector"} · score {reinvestment.score == null ? "—" : num(reinvestment.score, 0)} · rank #{reinvestment.rank ?? "—"}</p></div></div></div>}{recommendation.pct == null ? <p className="mt-3 rounded-2xl border border-[#ddb159]/16 bg-[#061b12]/72 px-3 py-2 text-[11px] font-bold text-[#faf6f0]/55">No automatic action is suggested for this holding.</p> : <button type="button" disabled={isPending} onClick={() => runPlan(recommendation.pct ?? 0)} className="mt-3 inline-flex h-10 items-center justify-center rounded-full bg-[linear-gradient(180deg,#f3d98b,#d6ae4d_55%,#a77d2e)] px-4 text-[11px] font-black uppercase tracking-[0.1em] text-[#061b12] shadow-[0_14px_30px_rgba(0,0,0,0.2)] disabled:opacity-50">Trim and reinvest</button>}</div><div className="rounded-2xl border border-[#ddb159]/14 bg-[#faf6f0]/[0.045] p-3"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#faf6f0]/42">Custom adjustment</p><div className="mt-2 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]"><input type="number" min={1} max={100} step={0.5} value={customPercent} onChange={(event) => setCustomPercent(event.target.value)} placeholder="Enter %" className="h-11 w-full rounded-2xl border border-[#ddb159]/18 bg-[#020805]/55 px-3 text-[14px] font-black text-[#faf6f0] outline-none placeholder:text-[#faf6f0]/32 focus:border-[#ddb159]" /><button type="button" disabled={isPending} onClick={() => runPlan(Number(customPercent))} className="h-11 rounded-2xl bg-[linear-gradient(180deg,#f3d98b,#d6ae4d_55%,#a77d2e)] px-5 text-[11px] font-black uppercase tracking-[0.1em] text-[#061b12] disabled:opacity-50">Apply %</button></div></div><div className="grid gap-2 sm:grid-cols-2"><button type="button" disabled={isPending} onClick={() => runRemove(true)} className="h-11 rounded-2xl border border-red-400/30 px-4 text-[11px] font-black uppercase tracking-[0.1em] text-red-200 disabled:opacity-50">Close + credit cash</button><button type="button" disabled={isPending} onClick={() => runRemove(false)} className="h-11 rounded-2xl border border-[#ddb159]/16 px-4 text-[11px] font-black uppercase tracking-[0.1em] text-[#faf6f0]/55 disabled:opacity-50">Remove only</button></div>{message && <p className="rounded-2xl bg-[#faf6f0]/[0.055] px-3 py-2 text-[11px] font-bold text-[#faf6f0]/62">{message}</p>}</div></div></div>, document.body);
}
...
function HoldingsRow({ portfolioId, holding, currency, riskTolerance, stockOptions, heldTickers }: { portfolioId: string; holding: ExtendedHolding; currency: string; riskTolerance: string | null; stockOptions: StockOption[]; heldTickers: Set<string> }) {
  ...
  return <div className="group relative overflow-hidden border-b border-[#072116]/8 bg-[#faf6f0] text-[#072116] transition hover:bg-white lg:rounded-2xl lg:border lg:shadow-[0_8px_22px_rgba(0,0,0,0.10)] lg:hover:border-[#ddb159]/45"><div className="absolute inset-y-0 left-0 hidden bg-[#ddb159]/10 transition group-hover:bg-[#ddb159]/16 lg:block" style={{ width: `${widthPct}%` }} /><div className="relative grid gap-2 px-3 py-2.5 lg:grid-cols-[minmax(210px,1.6fr)_110px_125px_90px_80px_94px] lg:items-center lg:px-4 lg:py-3"><div className="flex min-w-0 items-center gap-3"><StockLogo ticker={holding.ticker} size={34} /><div className="min-w-0"><p className="truncate text-[14px] font-black leading-none">{holding.ticker}</p><p className="mt-1 truncate text-[11px] font-semibold text-[#072116]/45">{holding.company ?? "Holding"}</p></div></div><HoldingCell label="Value" value={money(holding.currentValue, currency)} sub={`${num(holding.shares, 4)} sh`} /><HoldingCell label="Net P/L" value={money(holding.totalPnLDollars, currency)} sub={pct(holding.pnlPercent)} tone={isPositive ? "positive" : "negative"} /><HoldingCell label="%" value={`${holding.currentAllocationPct.toFixed(1)}%`} sub={`target ${holding.targetAllocationPct?.toFixed(1) ?? "—"}%`} /><HoldingCell label="AI" value={num(holding.score, 0)} sub={`#${holding.rank ?? "—"}`} tone="gold" /><div className="flex justify-end"><button type="button" onClick={() => setOpen(true)} className="h-9 rounded-full bg-[#072116] px-3 text-[10px] font-black uppercase tracking-[0.1em] text-[#ddb159] transition hover:brightness-110">Manage</button></div></div>{open && <ManageHoldingModal portfolioId={portfolioId} holding={holding} recommendation={trimRecommendation} onClose={() => setOpen(false)} />}</div>;
}
...
function ManagePanel({ portfolioId, portfolioName }: { portfolioId: string; portfolioName: string }) {
  ...
  function saveName() { const cleanName = name.trim(); if (!cleanName) { setMessage("Portfolio name cannot be empty."); return; } startRenaming(async () => { const result = await renamePortfolio({ portfolioId, name: cleanName }); if (!result.success) { setMessage(result.error ?? "Could not rename portfolio."); return; } setMessage("Portfolio renamed."); router.refresh(); }); }
  function runDelete() { if (!window.confirm(`Delete "${portfolioName}"? This removes this portfolio and its holdings only.`)) return; startDeleting(async () => { const result = await deletePortfolio({ portfolioId }); if (!result.success) { setMessage(result.error ?? "Could not delete portfolio."); return; } router.push("/portfolio"); router.refresh(); }); }
  return <div className="rounded-2xl bg-[#faf6f0] p-4 text-[#072116]"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#072116]/45">Manage portfolio</p><h3 className="mt-1 text-[20px] font-black tracking-[-0.04em]">Rename or delete</h3><div className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto_auto]"><input value={name} maxLength={80} onChange={(event) => setName(event.target.value)} className="h-11 min-w-0 rounded-2xl border border-[#072116]/10 bg-white px-3 text-[14px] font-black outline-none focus:border-[#ddb159]" /><button type="button" onClick={saveName} disabled={isRenaming || name.trim() === portfolioName.trim()} className="h-11 rounded-2xl bg-[#ddb159] px-5 text-[11px] font-black uppercase tracking-[0.1em] text-[#072116] disabled:opacity-60">{isRenaming ? "Saving…" : "Rename"}</button><button type="button" onClick={runDelete} disabled={isDeleting} className="h-11 rounded-2xl border border-red-500/35 px-5 text-[11px] font-black uppercase tracking-[0.1em] text-red-700 disabled:opacity-60">{isDeleting ? "Deleting…" : "Delete"}</button></div>{message && <p className="mt-2 rounded-xl bg-white px-3 py-2 text-[11px] font-bold text-[#072116]/65">{message}</p>}</div>;
}
...
export function PortfolioCommandCentreRevolut({ portfolioId, portfolios, holdings, stockOptions = [], transactions = [], chartData = {}, chartMeta, portfolioMeta, compactImportWidget }: Props) {
