export function DemoPortfolioBuildView() {
  const preferences = [
    ...
    ["Portfolio style", "Diversified research draft"],
  ];
    ...
    <div className="grid min-w-0 gap-3">
      <section className="rounded-3xl border border-[#ddb159]/25 bg-[linear-gradient(160deg,#0d3420,#082519)] p-5">
        <p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#ddb159]">
          Portfolio Draft builder
        </p>
        <h1 className="mt-2 text-3xl font-black tracking-[-0.05em]">
          Build a draft around your preferences.
        </h1>
        <p className="mt-2 max-w-2xl text-xs font-semibold leading-5 text-white/55">
          Choose objective, risk preference and time horizon, then review the
          allocation and trade-offs before deciding what deserves more research.
        </p>
      </section>
        ...
        <section className="rounded-2xl bg-[#faf6f0] p-5 text-[#072116]">
          ...
          <div className="mt-4 grid gap-2">
            {preferences.map(([label, value]) => (
              <div
                key={label}
                className="rounded-2xl border border-[#072116]/9 bg-white p-3"
              >
                <p className="text-[9px] font-black uppercase text-[#072116]/42">
                  {label}
                </p>
        ...
        <section className="rounded-2xl bg-[#faf6f0] p-5 text-[#072116]">
          ...
          <div className="mt-4 grid gap-2">
            {[
              ...
              <div
                key={sector}
                className="grid grid-cols-[minmax(0,1fr)_54px] gap-3 rounded-2xl border border-[#072116]/8 bg-white p-3"
              >
                <div>
                  <p className="font-black">{sector}</p>
                  <p className="mt-1 text-[10px] font-semibold text-[#072116]/48">
                    {detail}
                  </p>
                </div>
          ...
          <p className="mt-4 text-[10px] font-semibold leading-5 text-[#072116]/48">
            Portfolio Drafts are research starting points, not automatic
            investing instructions or personalised financial advice.
          </p>
        </section>
      </div>
    </div>
