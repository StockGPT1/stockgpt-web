...
function MetricCard({
  ...
  return (
    <div className="rounded-2xl border border-[#072116]/8 bg-white p-3">
      <p className="text-[9px] font-black uppercase tracking-[0.12em] text-[#072116]/42">
        {label}
      </p>
...
export function DemoStockView({
  sectionRefs,
}: {
  sectionRefs: DemoStockSectionRefs;
}) {
  return (
    <div className="grid min-w-0 gap-3">
      ...
      >
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-[10px] font-black text-[#ddb159]/70">
              Rankings · #1 · Technology
            </p>
            <div className="mt-3 flex items-center gap-3">
              <div className="grid size-11 place-items-center rounded-full bg-[#faf6f0] text-sm font-black text-[#072116]">
                N
              </div>
              <div>
                <h1 className="text-4xl font-black tracking-[-0.05em]">NVDA</h1>
                <p className="font-bold text-[#faf6f0]/64">
                  NVIDIA Corporation
                </p>
              </div>
            </div>
            <div className="mt-4 flex flex-wrap items-center gap-2">
              ...
              <span className="rounded-full bg-[#ddb159] px-3 py-1 text-[10px] font-black uppercase text-[#072116]">
                AI score · 8,742
              </span>
              <span className="rounded-full border border-[#ddb159]/30 px-3 py-1 text-[10px] font-black uppercase text-[#ddb159]">
          ...
          <div className="grid grid-cols-2 gap-2 sm:w-64">
            <div className="rounded-2xl border border-white/10 bg-white/[0.05] p-3">
              <p className="text-[9px] font-black uppercase text-white/38">
                1D move
              </p>
      ...
      <section className="rounded-2xl bg-[#faf6f0] p-4 text-[#072116]">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-3">
          <MetricCard
            ...
            value="Strong"
            note="Demand remains high, with execution risk."
          />
          <MetricCard
            label="Valuation context"
            value="Elevated"
            note="Expectations leave less room for error."
          />
          <MetricCard
            label="Risk context"
            value="Medium"
            note="Volatility can rise around results and guidance."
          />
        </div>
      </section>
      ...
      >
        ...
        <h2 className="mt-1 text-2xl font-black tracking-[-0.04em]">
          Relevant news affecting NVDA
        </h2>
        <p className="mt-2 max-w-3xl text-xs font-semibold leading-5 text-white/55">
          Headlines are connected to the company so you can check material
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-2">
          {demoNews.map((article) => (
            <article
              key={article.title}
              className="rounded-2xl border border-[#ddb159]/14 bg-[#0b2b1d] p-4"
            >
              <div className="flex flex-wrap gap-2 text-[9px] font-black uppercase text-[#ddb159]">
                <span>{article.source}</span>
                <span className="rounded-full bg-[#ddb159]/12 px-2 py-0.5">
                  {article.relevance}
                </span>
              </div>
      ...
      >
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <MetricCard
            label="Review area"
            value="$168-$174"
            note="Planning reference, not an instruction."
          />
          <MetricCard
            label="Downside review"
            value="$154"
            note="Reassess if the thesis weakens."
          />
          <MetricCard
            label="Upside scenario"
            value="$196"
            note="A scenario, not a promised target."
          />
        </div>
        <div className="mt-3 rounded-2xl bg-[#072116] p-4 text-[#faf6f0]">
          ...
          <p className="mt-2 text-xs font-semibold leading-5 text-white/60">
            If guidance weakens or margins compress, review the thesis. If
            demand and free cash flow support expectations, continue monitoring
            valuation and concentration risk.
          </p>
        </div>
        <p className="mt-3 text-[10px] font-semibold text-[#072116]/50">
      ...
      >
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 lg:grid-cols-4">
          <MetricCard
            ...
            value="31%"
            note="Illustrative annual growth"
          />
          <MetricCard
            label="Gross margin"
            value="72%"
            note="Illustrative profitability"
          />
          <MetricCard
            label="Forward P/E"
            value="34×"
            note="Valuation needs context"
          />
          <MetricCard
            label="Free cash flow"
            value="$42bn"
            note="Illustrative cash generation"
          />
        </div>
