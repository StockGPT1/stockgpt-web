...
| 05 | Portfolio correctness and persistence cleanup | IN PROGRESS | `05A design` + `05B ownership foundation` + `05C append-only ledger` | Stage 05A design, 05B database ownership foundation and 05C append-only ledger/occurred-at semantics are complete; accounting mutation slices remain. |
...
- Stage 05D cash/contributed-capital mutation is the next slice. Trade, creation, CSV, currency, direct-write retirement and cache work remain in their approved 05E–05J slices.
