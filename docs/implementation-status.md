...
| 08 | Broker secret/security architecture | COMPLETE | `Build secure broker sync engine` | Per-user provider secrets live in Supabase Vault behind an unexposed private credential schema and backend-only exact-scope RPCs; browser/anon access, rotation and local revocation were tested. |
| 09 | SnapTrade sandbox integration | COMPLETE | `Build secure broker sync engine` | Pinned official SDK adapter implements immutable user identity, read-only portal links and fine-grained connection/account/position/balance/activity normalization. Deterministic provider-contract fixtures pass; external test-key sandbox network smoke is pending. |
| 10 | Broker sync state machine and reconciliation | COMPLETE | `Build secure broker sync engine` | Durable bounded leases, candidate validation and one trusted atomic promotion boundary preserve last-good normalized state on incomplete/provider/database failure. |
...
- Step 05B deliberately retained temporary direct authenticated Portfolio/holding writes and a transaction INSERT column allowlist while narrow mutation RPCs were introduced. Step 05I has now retired that compatibility surface for authoritative state; derived snapshot privileges remain separately owned by 05J.
...
- Step 05F moves manual Portfolio creation, AI Portfolio Draft save-as-new and exact lifecycle deletion into three narrow authenticated `SECURITY DEFINER` RPCs. Each derives the owner from `auth.uid()`, creates only USD normal Portfolios, and atomically owns the parent/holdings/initial-ledger lifecycle; exact deletion locks and removes only the requested owned Portfolio while existing FK cascades handle child lifecycle rows.
- Manual initialization derives net contribution from starting cash plus persisted holding cost bases, emits one unknown-occurrence starting-cash deposit when applicable and one unknown-occurrence `log_existing` row per holding. AI initialization starts with zero cash, derives contribution from actual persisted holding basis rather than the requested `totalInvested`, and records one neutral `import` row rather than fabricated buys.
- `investment_amount` remains DB-derived holdings-basis compatibility state only and is not accounting authority. Initial captured state does not fabricate event timestamps; `created_at` records when StockGPT stored it. Forced local holding/ledger failures prove the parent, holdings and ledger roll back together.
- Active creation/deletion actions no longer split financial writes or expose AI replacement, arbitrary storage currency, optional deletion, first-Portfolio fallback or implicit creation. Post-commit snapshot/cache/revalidation failures cannot turn a committed creation/deletion into a reported financial failure.
- Step 05G gives Trading 212 CSV preview and commit one shared deterministic parser/mapping contract. Investment rows are processed chronologically with weighted remaining cost basis, known non-investment activity is ignored explicitly, and malformed, ambiguous or unsupported open positions refuse the whole file rather than being silently dropped.
- New Trading 212 portfolios are created atomically as USD with zero imported cash, net contribution derived from persisted open-position basis and one neutral unknown-occurrence `import` ledger row. Existing imports are exact holdings replacement on one owned locked USD Portfolio: cash, net contribution and prior ledger history remain unchanged, while one neutral replacement audit row is appended.
- Both CSV mutations are narrow authenticated `SECURITY DEFINER` RPCs. They validate the complete holdings payload before mutation; concurrent cash/holding/import operations serialize on the Portfolio row; forced ledger failures roll back parent/holdings changes; repeated identical replacement is financially idempotent. Preview state is never trusted at commit, and post-commit cache/revalidation remains best-effort derived work.
- Active CSV paths no longer split-write financial tables or offer merge behavior. Remaining broad authenticated direct-table privileges and dead wrappers remain owned by 05I; no legacy currency conversion, global privilege retirement or cache reconciliation semantics were pulled forward.
- Step 05H establishes exact stored `USD` as the canonical accounting basis for normal Portfolios. GBP/EUR/CHF remain display-only preferences for canonical Portfolios, and changing `profiles.preferred_currency` cannot mutate Portfolio, holding or ledger financial facts. Unknown, blank and non-USD stored Portfolio currencies never default to USD accounting.
- Existing legacy non-USD money is preserved unchanged and classified as ambiguous. Current or fallback FX is never used to repair it; mixed-unit P&L, Health, concentration, chart and snapshot calculations are suppressed, financial mutations fail closed, and safe rename/preferences/exact-delete lifecycle access remains available.
- The modern Portfolio presents a limited legacy-currency state with only safe identity/share facts and recreation/deletion choices. Dashboard and Ask withhold ambiguous monetary facts, canonical Notifications remain limited, and page/cache/snapshot jobs skip legacy currency portfolios rather than writing misleading derived values.
- Authenticated database enforcement rejects new non-USD normal Portfolio rows and prevents legacy currency/financial-column rewrites. The temporary exact-owned canonical-USD child DML policies from 05H are retired by 05I because authenticated direct authoritative DML is no longer granted. No existing Portfolio or transaction row is rewritten.
- FX quotes now retain provenance. USD identity and valid configured/fetched current rates are write-safe; hard-coded fallback rates remain display-only and cannot convert financial input into stored truth. When a non-USD write-safe rate is unavailable, manual creation and cash/holding controls refuse the write instead of guessing.
- Step 05I defines `user_portfolios`, `portfolio_holdings` and `portfolio_transactions` as authoritative Portfolio state. Authenticated customer table access is SELECT-only under one exact owned-read RLS policy per table; anon has no direct access, all table/column mutation privileges are removed, and the 05C direct ledger INSERT allowlist is retired. Trusted service-role table capability remains available for deliberate admin/test infrastructure.
- All active authoritative customer writes now use narrow purpose-specific `SECURITY DEFINER` functions that derive ownership from `auth.uid()` and exact Portfolio identity. Rename, preferences and holding-review metadata use the new `rename_owned_portfolio`, `update_owned_portfolio_preferences` and `mark_portfolio_holding_reviewed` RPCs; the no-ID `markReviewed` overload, first-Portfolio fallback and unused direct-write helper are removed.
- The holding trade-level GET route is side-effect free for authoritative state: it may calculate transient compatibility levels but no longer persists `risk_level_at_entry` or `target_level_at_entry`. A source/behavior boundary scan covers Portfolio, Dashboard, Ask, Notifications and other runtime reads and rejects direct authoritative INSERT/UPDATE/DELETE/UPSERT.
- Whole-Portfolio deletion remains the exact `delete_owned_portfolio` lifecycle RPC and preserves FK cascades without reopening direct DELETE. Safe legacy-currency rename/preferences/review/delete operations remain available through narrow RPCs, while every financial RPC still fails closed for ambiguous non-USD state.
- Step 05J keeps `user_portfolios`, `portfolio_holdings` and `portfolio_transactions` as the only authoritative current Portfolio state. Current page accounting derives from those Stage 05 facts plus current market facts; `cash_deposited_total` remains exact net contributed capital, including zero and negative values, and `investment_amount` is excluded from accounting and cache truth.
- Current valuation now fails honestly when cash, contribution or a positive-share holding's current price is unavailable or invalid. Missing prices are never replaced by zero or entry price, and return percentage is unavailable when net contribution is zero or negative.
- `portfolio_snapshots` remains semantic history, distinct from disposable page/cache state. Ordinary page reads, cache misses and post-mutation cache repair no longer append semantic history; existing rows remain untouched, while dedicated canonical-USD live capture, health repair and historical backfill writers remain isolated for Stage 14 historical-performance reconciliation.
...
- Stage 06 establishes `instruments.id` as StockGPT's permanent UUID for one specific tradable listing. `instrument_aliases` stores ticker and provider mappings in explicit namespace/scope pairs with non-overlapping validity windows, so symbol changes do not change identity, expired aliases can be reused without deleting history, and similarly named/tickered listings on different venues do not collapse. Resolution requires one unambiguous alias at an explicit time.
...
- Stage 07 separates global broker provider and brokerage institution catalogs from user-owned provider connections and broker accounts. Provider-local connection IDs are unique per StockGPT owner/provider, not assumed globally unique across users. Broker accounts are not StockGPT Portfolios and are not projected into `user_portfolios`; that remains Stage 13.
...
- At Wave 1 closeout, a clean local reset, 36-table/RLS fixture contract, Wave 1 catalog/security checks, focused instrument and broker tests, the complete Stage 05 acceptance suite, generated-type drift, Portfolio/Stage 04 regression, Next type generation, TypeScript, lint and guarded production build passed. Stage 06 and Stage 07 remain complete; the subsequent Wave 2 work is recorded below.

### Wave 2 — Stages 08, 09 and 10

- Stage 08 keeps commercial SnapTrade client ID/consumer key in backend environment configuration only. Per-user `userSecret` values are stored in Supabase Vault; an unexposed `broker_private` schema holds only exact user/provider metadata and a Vault reference. Fixed empty-search-path public `SECURITY DEFINER` RPCs have explicit service-role-only EXECUTE grants for store, exact retrieval and revocation. Normal authenticated/anon sessions cannot read the private schema or call those RPCs. Vault rotation and local Auth-cascade/revocation deletion are proven with synthetic credentials; provider-side deregistration remains an explicit later connection/account-lifecycle concern, not an implicit database cascade.
- The proposed dedicated `stockgpt_broker_backend` role was not compatible with the pinned local Supabase migration runner: migrations run as reserved `supabase_admin`, which cannot safely acquire or SET ROLE to a newly created backend role. The narrower audited definer boundary above avoids changing platform-managed roles. No broker secret or raw provider payload is stored in a public customer table, job, log or test snapshot.
- Stage 09 pins `snaptrade-typescript-sdk` 12.2.7. The backend adapter derives an immutable provider user ID from the StockGPT UUID, supports user registration/secret persistence and rotation, forces `connectionType = "read"` for portal links, and fetches connections, accounts, fine-grained positions, balances and paginated activities. Alias resolution uses the Stage 06 namespaced resolver; ambiguous or missing mappings keep the position with a null `instrument_id`. Multi-currency cash and provider prices remain account evidence, never global market truth. Disabled, holdings-unavailable, incomplete-initial-sync and endpoint failures cannot become confirmed empty state. No trading/manual-refresh endpoint is used. No valid non-production SnapTrade test key was available, so no external sandbox network smoke is claimed; a test-key read-only smoke remains pending before real connection UX/pilot work.
- Stage 10 stores only scoped status/lease/freshness/sanitized failure metadata in durable `broker_sync_jobs`. Backend-only enqueue, bounded `FOR UPDATE SKIP LOCKED` claim, failure and atomic promotion RPCs prevent browser ingestion. The provider-neutral worker follows queue → fetch → normalize → validate → promote with bounded account/activity windows. Promotion locks the exact job and connection, checks ownership, lease, completeness and freshness, reconciles complete positions/per-currency balances, appends activities idempotently and updates success metadata only in the same successful transaction. Failure preserves last-good positions, cash and activities; confirmed empty may clear only authoritative complete facts. Disconnected/revoked connections cannot erase normalized history.
- The new forward migrations apply on a clean LOCAL seeded reset. The 37-public-table/RLS contract, Vault hostile-session tests, Wave 1 owner/identity regressions, fake-provider endpoint/rollback/concurrency/replay tests, migration safety/negative checks, generated-type drift, complete Stage 05 acceptance, Portfolio/Stage 04 regression, Next type generation, TypeScript, tracked-source lint and guarded build pass without production/provider calls. The subsequent Wave 3 customer connection, onboarding and connected-Portfolio work is recorded below.

### Wave 3 — Stages 11, 12 and 13

- **Stage 11 COMPLETE.** Broker connection capability is server-owned, reuses the active-subscriber entitlement and defaults off unless `STOCKGPT_BROKER_CONNECTIONS_ENABLED=true`. The authenticated backend owns registration and secret lookup, forces the pinned SnapTrade SDK portal to read-only access, returns an ephemeral portal redirect without persisting or logging it, and never exposes provider secrets to the browser. The return route ignores callback identity claims and performs provider discovery through the exact stored user credential. Ordinary connection and Portfolio page rendering reads StockGPT database state only and never calls SnapTrade.
- **Stage 12 COMPLETE.** Provider discovery resolves institutions through provider ID plus the provider's external institution identity, never display-name matching. Provider-neutral connection/account state is upserted under the exact owner and initial sync is queued through the existing Stage 10 engine. The onboarding surface distinguishes awaiting discovery, syncing, connected, stale/error, revoked and disconnected/reconnect states; incomplete sync is not presented as an empty or zero-value account, and disconnect preserves last-good normalized facts. A valid non-production SnapTrade key is still unavailable, so external sandbox smoke remains explicitly pending and no external provider call is claimed.
- **Stage 13 COMPLETE.** `user_portfolios.management_source` separates `manual` and `connected` Portfolios, with one idempotent exact-owner mapping from one broker account to one connected Portfolio. A centralized database guard prevents manual holdings/ledger writes and all Stage 05 financial mutation RPCs from changing connected Portfolio financial state, while rename/preferences remain available. Deleting the StockGPT Portfolio preserves broker account/history. Connected facts come from normalized broker positions, per-currency cash, sync metadata and permanent instrument mappings; nothing is copied into `portfolio_holdings`.
- Connected valuation uses valid provider account evidence and the existing safe current-FX boundary. Unsupported instruments remain visible, unknown price/cash/FX never becomes zero, and incomplete material cash/valuation suppresses concentration conclusions. Connected aggregate performance remains explicitly unavailable until Stage 14; no cost basis, contribution return, total P&L or flat history is fabricated.
- Portfolio, Dashboard, Ask StockGPT and Notifications all consume the same source-aware Stage 04 canonical intelligence boundary. The four canonical statuses remain unchanged, broker facts never become global market facts, general reads make no provider calls, and no transaction recommendation/trading capability was added. Manual Portfolio and Stage 05 accounting behavior remain unchanged.
- A clean LOCAL reset, migration safety/negative checks, generated-type drift, Wave 1 and Wave 2 regressions, Wave 3 entitlement/ownership/read-only/projection checks, complete Stage 05 acceptance, affected Stage 04/Portfolio regressions, Next type generation, TypeScript, tracked-source lint and guarded build pass without production/provider access. **Stage 14 remains NOT STARTED and is the next major stage.**
