...
| 04 | Canonical portfolio intelligence engine | IN PROGRESS | Stage 04A audit + 04B canonical V1 engine + 04C factual adapter | Pure structured assessment boundary, deterministic contract suite and authenticated current-schema factual adapter are proven locally; customer-surface migration remains outstanding. |
...
- The recommended next slice is a reviewed Portfolio-only migration that reuses the page's existing owned reads, keeps canonical USD facts ahead of display conversion and temporarily leaves canonical status independent of news. A dedicated news-to-event severity adapter requires a separate approved semantic decision.
