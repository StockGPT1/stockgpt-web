...
| 05 | Portfolio correctness and persistence cleanup | IN PROGRESS | `05A design` + `05B ownership foundation` + `05C append-only ledger` + `05D atomic cash` | Stage 05A design, 05B database ownership, 05C append-only ledger/occurred-at semantics and 05D atomic cash/net-contribution mutation are complete; holding/trade and later persistence slices remain. |
...
- Direct authenticated financial-table access needed by remaining buy/sell/creation/import paths deliberately remains until 05I; active cash paths no longer use it. No 05E buy/sell semantics were pulled forward.
- Stage 05E holdings purchases/sales/remove is the next slice. Creation, CSV, full currency reconciliation, direct-write retirement and cache work remain in their approved 05F–05J slices.
