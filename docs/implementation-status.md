...
| 05 | Portfolio correctness and persistence cleanup | IN PROGRESS | `05A design` + `05B ownership foundation` + `05C append-only ledger` + `05D atomic cash` + `05E atomic holdings` + `05F atomic creation/lifecycle` + `05G atomic Trading 212 import` + `05H USD accounting basis` | Stage 05A design and 05B–05H persistence/currency foundations are complete; direct-write retirement and cache slices remain. |
...
- Derived `portfolio_snapshots`, `portfolio_page_snapshots`, chart/cache repair and cache-warming writes are non-authoritative and intentionally remain owned by 05J; no cache reconciliation was pulled into 05I.
- Stage 05I direct/dangerous authoritative-write retirement is complete. Stage 05J page/cache/snapshot reconciliation is next, followed by 05K final Stage 05 acceptance audit.
