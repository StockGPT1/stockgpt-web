...
function makeAlert(
  input: Omit<HoldingAlert, "id"> & { ticker: string },
): HoldingAlert {
...
function latestDiagnosticScore(diagnostics?: DiagnosticRecord | null) {
  ...
  return (
    ...
    nullableNumber(diagnostics.current_score)
  );
}
...
function negativeFactorEvidence(diagnostics?: DiagnosticRecord | null) {
  ...
  const diagnosis =
    typeof diagnostics.diagnosis === "string" ? diagnostics.diagnosis : null;

  if (
    diagnosis &&
    diagnosis !== "no single factor driver exceeded the diagnostic threshold"
  ) {
...
async function getSectorData(supabase: SupabaseQueryClient) {
  ...
  const scores = allStocks
    .map((stock) => finiteNumber(stock.score, 0))
    .filter((score) => score > 0);
  const maxScore = scores.length ? Math.max(...scores, 1) : 1;

  const bySector: Record<
    string,
    { total: number; bullish: number; healthy: number }
  > = {};
  ...
  allStocks.forEach((stock) => {
    ...
    if (!bySector[sector]) {
      bySector[sector] = { total: 0, bullish: 0, healthy: 0 };
    }

    bySector[sector].total += 1;

    if (rank > 0 && rank <= topQuartileRank) {
      bySector[sector].bullish += 1;
    }

    if (rank > 0 && rank <= topHalfRank) {
      bySector[sector].healthy += 1;
    }
  });
  ...
  Object.entries(bySector).forEach(([sector, value]) => {
    const bullish =
      value.total > 0 ? Math.round((value.bullish / value.total) * 100) : 0;
    const healthy =
      value.total > 0 ? Math.round((value.healthy / value.total) * 100) : 0;
...
function nearestBelow(values: number[], price: number) {
  const candidates = values.filter(
    (value) => Number.isFinite(value) && value > 0 && value < price * 0.995,
  );
...
function nearestAbove(values: number[], price: number) {
  const candidates = values.filter(
    (value) => Number.isFinite(value) && value > price * 1.005,
  );
...
async function getTechnicalLevels(
  ...
  try {
    ...
    const volatilityPct = Math.max(
      1.5,
      Math.min(7, (average(recentAbsMoves) ?? 0.02) * 100),
    );
    ...
    const resistance = nearestAbove(
      [...swingHighs, Math.max(...recent.slice(-126))],
      currentPrice,
    );

    const volatilityStop =
      currentPrice * (1 - Math.max(0.055, (volatilityPct * 2.2) / 100));
    const supportStop = support ? support * 0.99 : null;
    const stopLoss = Math.max(
      ...[volatilityStop, supportStop].filter(
    ...
    const takeProfit =
      resistance && resistance > currentPrice * 1.04
        ? resistance * 0.995
        : currentPrice * (1 + Math.max(0.1, (volatilityPct * 3.2) / 100));
  ...
  if (latestScorePct !== null && latestScorePct <= -8) {
    alerts.push(
      makeAlert({
        ...
        message: `The latest diagnostics show a ${Math.abs(
          latestScorePct,
        ).toFixed(1)}% score decline versus the previous run.`,
        recommendation:
          "Review the stock, but only act if the action alert also confirms deterioration.",
        evidence: [
          `Latest run score change: ${latestScorePct.toFixed(1)}%`,
          ...negativeEvidence.slice(0, 3),
        ],
        priority: 30,
        expiresWhen:
          "This alert disappears when the latest model-run score change is no longer material.",
      }),
    );
  }
  ...
  if (latestScorePct !== null && latestScorePct >= 10) {
    alerts.push(
      makeAlert({
        ...
        message: `The latest diagnostics show a ${latestScorePct.toFixed(
          1,
        )}% score improvement versus the previous run.`,
        recommendation:
          "No automatic action. This supports holding, and may support adding only if the action alert confirms it.",
        evidence: [
          `Latest run score change: +${latestScorePct.toFixed(1)}%`,
          ...positiveEvidence.slice(0, 3),
        ],
        priority: 70,
        expiresWhen:
          "This alert disappears when the improvement is no longer present in the latest run.",
      }),
    );
  }
  ...
  if (heldPastGrace && scoreFromEntryPct !== null && scoreFromEntryPct <= -15) {
    alerts.push(
      makeAlert({
        ...
        message: `The score is down ${Math.abs(scoreFromEntryPct).toFixed(
          1,
        )}% since you added the position.`,
        recommendation:
          "Review whether the original thesis still holds. This becomes actionable only with weak rank, price, news or factor confirmation.",
        evidence: [
          `Entry score: ${ctx.scoreAtEntry?.toLocaleString() ?? "—"}`,
          `Current score: ${ctx.score.toLocaleString()}`,
          ...negativeEvidence.slice(0, 2),
        ],
        priority: 35,
        expiresWhen:
          "This alert disappears when the score recovers above the material-deterioration threshold.",
      }),
    );
  }
  ...
  if (heldPastGrace && scoreFromEntryPct !== null && scoreFromEntryPct >= 18) {
    alerts.push(
      makeAlert({
        ...
        message: `The score is up ${scoreFromEntryPct.toFixed(
          1,
        )}% since you added the position.`,
        recommendation:
          "This supports holding. Only buy more if the separate action alert confirms the stock is still attractively positioned.",
        evidence: [
          `Entry score: ${ctx.scoreAtEntry?.toLocaleString() ?? "—"}`,
          `Current score: ${ctx.score.toLocaleString()}`,
  ...
  if (heldPastGrace && rankMove !== null && rankMove >= 60) {
    alerts.push(
      makeAlert({
        ...
        recommendation:
          "Review against stronger alternatives. This becomes a sell/trim only if the action logic confirms weak conviction.",
        evidence: [
          `Entry rank: #${ctx.rankAtEntry}`,
          `Current rank: #${ctx.rank}`,
          `Current rank percentile: ${ctx.rankPercentile}/100`,
        ],
        priority: 40,
        expiresWhen:
          "This alert disappears when the rank recovers enough that the move is no longer material.",
      }),
    );
  }
  ...
  if (heldPastGrace && rankMove !== null && rankMove <= -40) {
    alerts.push(
      makeAlert({
        ...
        message: `The stock moved from #${ctx.rankAtEntry} at entry to #${ctx.rank} now.`,
        recommendation:
          "Positive confirmation. Hold unless price or concentration risk says otherwise.",
        evidence: [
          `Entry rank: #${ctx.rankAtEntry}`,
          `Current rank: #${ctx.rank}`,
          `Current rank percentile: ${ctx.rankPercentile}/100`,
        ...
        priority: 80,
        expiresWhen:
          "This alert disappears when the ranking improvement is no longer material.",
      }),
    );
  }
  ...
  if (ctx.recentNegativeNewsCount >= 3) {
    alerts.push(
      makeAlert({
        ...
        recommendation:
          "Read the recent news before adding. If the AI signal is also weakening, this may become an action alert.",
        evidence: [
          `${ctx.recentNegativeNewsCount} negative articles`,
          ctx.latestNegativeHeadline
            ? `Latest: ${ctx.latestNegativeHeadline}`
            : "Recent negative coverage detected",
        ],
        priority: 32,
        expiresWhen:
          "This alert disappears when the negative news cluster leaves the recent-news window.",
      }),
    );
  }
  ...
  if (ctx.recentPositiveNewsCount >= 4) {
    alerts.push(
      makeAlert({
        ...
        evidence: [
          `${ctx.recentPositiveNewsCount} positive articles`,
          ctx.latestPositiveHeadline
            ? `Latest: ${ctx.latestPositiveHeadline}`
            : "Recent positive coverage detected",
        ],
        priority: 85,
        expiresWhen:
          "This alert disappears when the positive news cluster leaves the recent-news window.",
      }),
    );
  }
  ...
  if (
    ...
    alerts.push(
      makeAlert({
        ...
        recommendation:
          "Avoid adding. Consider rotation only if the action alert confirms weak stock-specific conviction.",
        evidence: [
          `Sector momentum: ${ctx.sectorMomentum}`,
          `Sector top-quartile participation: ${ctx.sectorBullishPct}%`,
          `Stock rank percentile: ${ctx.rankPercentile}/100`,
        ],
        priority: 55,
        expiresWhen:
          "This alert disappears when the sector backdrop improves or the stock regains strong relative rank.",
      }),
    );
  }
  ...
  if (
    ...
    alerts.push(
      makeAlert({
        ...
        recommendation:
          "Sector support improves the hold case. Buy more only if the action alert confirms the position is still underbuilt.",
        evidence: [
          `Sector momentum: ${ctx.sectorMomentum}`,
          `Sector top-quartile participation: ${ctx.sectorBullishPct}%`,
          `Stock rank percentile: ${ctx.rankPercentile}/100`,
        ...
        priority: 90,
        expiresWhen:
          "This alert disappears when sector breadth or stock rank weakens.",
      }),
    );
  }
  ...
  if (heldPastGrace && ctx.pnlPercent <= -12) {
    alerts.push(
      makeAlert({
        ...
        severity: "info",
        action: "review",
        title: `${ctx.ticker} is down ${Math.abs(ctx.pnlPercent).toFixed(
          1,
        )}% from entry`,
        message:
          "The price move is large enough to review, but price alone is not treated as a sell signal.",
        recommendation:
          "Check whether the AI score and rank still support the position.",
        evidence: [
          `Entry price: ${money(ctx.entryPrice)}`,
          `Current price: ${money(ctx.currentPrice)}`,
          `P&L: ${ctx.pnlPercent.toFixed(1)}%`,
        ],
        priority: 65,
        expiresWhen:
          "This alert disappears when the drawdown is no longer material.",
      }),
    );
  }
  ...
  if (heldPastGrace && ctx.pnlPercent >= 30) {
    alerts.push(
      makeAlert({
        ...
        recommendation:
          "Do not automatically sell winners. Trim only if the action alert flags concentration or weakening conviction.",
        evidence: [
          `Entry price: ${money(ctx.entryPrice)}`,
          `Current price: ${money(ctx.currentPrice)}`,
          `P&L: +${ctx.pnlPercent.toFixed(1)}%`,
...
function buildActionAlert(ctx: AlertContext, eventAlerts: HoldingAlert[]) {
  ...
  const targetUpsidePct =
    ctx.technical.takeProfit && ctx.technical.takeProfit > ctx.currentPrice
      ? ((ctx.technical.takeProfit - ctx.currentPrice) / ctx.currentPrice) * 100
      : 0;
  const sectorStrong =
    ctx.sectorMomentum === "Strong" || ctx.sectorMomentum === "Booming";
  ...
  const underbuilt =
    ctx.targetAllocationPct !== null
      ? ctx.currentAllocationPct <= Math.max(2, ctx.targetAllocationPct - 3)
  ...
  if (heldPastGrace && stopHit && (ctx.rankPercentile < 60 || clearDeterioration)) {
    return makeAlert({
      ...
      message: `Current price ${money(
        ctx.currentPrice,
      )} is at or below the active risk level of ${money(
        ctx.technical.stopLoss!,
      )}, and conviction is not strong enough to ignore it.`,
      recommendation:
        "Sell or materially reduce the position. Do not average down into a broken setup.",
      evidence: [
        `Current price: ${money(ctx.currentPrice)}`,
        `Risk level: ${money(ctx.technical.stopLoss!)}`,
        `Rank percentile: ${ctx.rankPercentile}/100`,
        ...(scoreFromEntryPct !== null
          ? [`Score from entry: ${scoreFromEntryPct.toFixed(1)}%`]
          : []),
        ...negativeEvidence.slice(0, 2),
      ],
      priority: 1,
      expiresWhen:
        "This action disappears if price recovers above the risk level or AI conviction recovers.",
    });
  }
  ...
  if (heldPastGrace && ctx.rankPercentile <= 25 && clearDeterioration) {
    return makeAlert({
      ...
      recommendation:
        "Sell the position and redeploy into a stronger-ranked stock. This is not just noise.",
      evidence: [
        `Rank percentile: ${ctx.rankPercentile}/100`,
        ctx.rank
          ? `Current rank: #${ctx.rank} of ${ctx.totalStocks}`
          : "Current rank unavailable",
        ...(scoreFromEntryPct !== null
          ? [`Score from entry: ${scoreFromEntryPct.toFixed(1)}%`]
          : []),
        ...(latestScorePct !== null
          ? [`Latest score move: ${latestScorePct.toFixed(1)}%`]
          : []),
        ...negativeEvidence.slice(0, 2),
      ],
      priority: 2,
      expiresWhen:
        "This action disappears if the stock moves out of the bottom quartile or deterioration is no longer confirmed.",
    });
  }
  ...
  if (
    ...
    (negativeEvidence.length > 0 || ctx.recentNegativeNewsCount >= 2)
  ) {
    return makeAlert({
      ticker: ctx.ticker,
      category: "action",
      ...
      message: `The score is down ${Math.abs(scoreFromEntryPct).toFixed(
        1,
      )}% since entry and the current rank is no longer strong enough to defend it.`,
      recommendation:
        "Exit or cut heavily. Reassess only if the score and rank recover.",
      evidence: [
        `Score from entry: ${scoreFromEntryPct.toFixed(1)}%`,
        `Rank percentile: ${ctx.rankPercentile}/100`,
        ...negativeEvidence.slice(0, 3),
      ...
      priority: 3,
      expiresWhen:
        "This action disappears if the score/rank recovery invalidates the sell condition.",
    });
  }
  ...
  if (overbuilt) {
    return makeAlert({
      ...
      priority: 10,
      expiresWhen:
        "This action disappears once the position is no longer materially oversized.",
    });
  }
  ...
  if (
    ...
    return makeAlert({
      ...
      message: `The score has fallen ${Math.abs(scoreFromEntryPct).toFixed(
        1,
      )}% since entry and rank is no longer high-conviction.`,
      recommendation:
        "Trim rather than fully exit. Keep a smaller position only if the thesis still makes sense.",
      evidence: [
        `Score from entry: ${scoreFromEntryPct.toFixed(1)}%`,
        `Rank percentile: ${ctx.rankPercentile}/100`,
        ...negativeEvidence.slice(0, 2),
  ...
  if (heldPastGrace && ctx.pnlPercent >= 35 && ctx.rankPercentile < 75) {
    return makeAlert({
      ...
      message: `${ctx.ticker} is up ${ctx.pnlPercent.toFixed(
        1,
      )}%, but the AI rank is no longer top-tier.`,
      recommendation:
        "Trim part of the winner and keep a core position only if rank stabilises.",
      evidence: [
        `P&L: +${ctx.pnlPercent.toFixed(1)}%`,
        `Rank percentile: ${ctx.rankPercentile}/100`,
        ctx.technical.takeProfit
          ? `Nearest target area: ${money(ctx.technical.takeProfit)}`
          : "Technical target unavailable",
      ],
      priority: 14,
      expiresWhen:
        "This action disappears if the rank improves or the position is no longer an extended winner.",
    });
  }
  ...
  if (
    ...
    return makeAlert({
      ...
      recommendation:
        "Consider adding gradually. Do not chase with one large order.",
      evidence: [
        `Rank percentile: ${ctx.rankPercentile}/100`,
        `Sector momentum: ${ctx.sectorMomentum}`,
        `Current allocation: ${ctx.currentAllocationPct.toFixed(1)}%`,
        `Estimated upside to target zone: ${targetUpsidePct.toFixed(1)}%`,
        ...positiveEvidence.slice(0, 2),
      ],
      priority: 20,
      expiresWhen:
        "This action disappears if rank, sector strength, allocation gap, upside, or news confirmation no longer supports adding.",
    });
  }

  const warningEventCount = eventAlerts.filter(
...
function buildTriggers(ctx: AlertContext): HoldingTrigger[] {
  ...
  if (ctx.technical.stopLoss) {
    triggers.push({
      ...
      action:
        ctx.rankPercentile >= 80
          ? "Review first because AI conviction is still strong, but do not ignore a clean break."
          : "Treat this as an invalidation level and consider selling or cutting heavily.",
    });
  }
  ...
  if (ctx.technical.takeProfit) {
    triggers.push({
      ...
      tone: "positive",
      priority: 2,
      condition: `If ${ctx.ticker} approaches ${money(
        ctx.technical.takeProfit,
      )}`,
      action:
        ctx.rankPercentile >= 85
          ? "Do not fully sell an elite-ranked stock. Consider trimming only if oversized."
          : "Consider trimming part of the position and reassessing the remaining stake.",
    });
  }
  ...
  if (ctx.scoreAtEntry) {
    const scoreFloor =
      ctx.rankPercentile >= 85
        ? Math.round(ctx.scoreAtEntry * 0.78)
        : ctx.rankPercentile >= 70
          ? Math.round(ctx.scoreAtEntry * 0.74)
          : Math.round(ctx.scoreAtEntry * 0.7);
...
function deriveRecommendation(
  ...
  if (primaryAction?.action === "sell") {
    return ctx.currentAllocationPct < 5 ? "Sell Whole Position" : "Sell Immediately";
  }
...
function buildAISummary(
  ...
  if (primaryAction) {
    return `${primaryAction.title}. ${primaryAction.recommendation}`;
  }

  const warningEvents = eventAlerts.filter(
    (alert) => alert.severity === "warning" || alert.severity === "critical",
  ).length;

  const positiveEvents = eventAlerts.filter(
  ...
  if (warningEvents > 0) {
    return `${ctx.ticker} has ${warningEvents} event alert${
      warningEvents === 1 ? "" : "s"
    }, but no action alert. Something changed, but the evidence is not strong enough to recommend buying, trimming or selling.`;
  }
  ...
  return `${ctx.ticker} has no active action signal. Current rank percentile is ${ctx.rankPercentile}/100, allocation is ${ctx.currentAllocationPct.toFixed(
    1,
...
async function enrichHoldingsWithClient(
  ...
  ((recentNews ?? []) as RecentNewsRow[]).forEach((article) => {
    ...
    affectedTickers.forEach((ticker: string) => {
      ...
      if (impact === "positive") {
        ...
        if (!newsByTicker[ticker].latestPositiveHeadline && title) {
          newsByTicker[ticker].latestPositiveHeadline = title;
        }
      }
      ...
      if (impact === "negative") {
        ...
        if (!newsByTicker[ticker].latestNegativeHeadline && title) {
          newsByTicker[ticker].latestNegativeHeadline = title;
        }
      }
    });
  });
  ...
  return await Promise.all(
    holdings.map(async (holding) => {
      ...
      const holdingStartDate = safeDate(
        holding.purchase_date ?? holding.added_at,
        holding.added_at,
      );
      const addedDate = safeDate(holding.added_at, new Date().toISOString());
      const reviewedDate = safeDate(holding.last_reviewed_at, holding.added_at);

      const daysHeld = daysBetween(now, holdingStartDate);
      const daysSinceAddedToStockGPT = daysBetween(now, addedDate);
      const daysSinceReview = daysBetween(now, reviewedDate);
      const isRecentlyAdded =
        daysSinceAddedToStockGPT < RECENT_HOLDING_GRACE_DAYS;
      ...
      const scorePercentile = Math.max(
        0,
        Math.min(100, Math.round((score / Math.max(maxScore, 1)) * 100)),
      );
      const rankPercentile = rank
        ? Math.max(
            0,
            Math.min(
              100,
              Math.round(
                (1 - (rank - 1) / Math.max(totalStocks - 1, 1)) * 100,
              ),
            ),
          )
        : scorePercentile || 50;
