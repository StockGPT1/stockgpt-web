...
export function buildPortfolioTrimRecommendation(
  ...
  const drivers: PortfolioTrimDriver[] = [
    ...
    {
      name: "Alerts / news",
      detail: holding.actionAlerts[0]?.title ?? `${holding.eventAlerts.length} active event alerts`,
      pct: alertTrimPct,
      weight: 20,
    },
    {
  ...
  const rawTrimPct = Math.max(
    allocationTrimPct,
    convictionTrimPct,
    alertTrimPct,
    profitProtectionTrimPct,
  );
  const riskScore = Math.round(
    clamp(
      (allocationTrimPct / 60) * 40 +
        (convictionTrimPct / 35) * 30 +
        (alertTrimPct / 40) * 20 +
        (profitProtectionTrimPct / 30) * 10,
      0,
      100,
    ),
  );
  ...
  if (!Number.isFinite(rawTrimPct) || rawTrimPct < 5 || holding.currentValue * (rawTrimPct / 100) < 25) {
    return {
      ...
      reason:
        "Allocation is inside range, AI conviction is not weak enough, and no confirmed trim signal is active.",
      riskScore,
      estimatedValue: null,
