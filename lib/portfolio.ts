...
export type Portfolio = {
  ...
  // ── NEW: AI Analysis ──
  strategy: string; // overall narrative
  expectedAnnualReturn: number; // %
  riskAssessment: string; // sentence
  diversificationScore: number; // 0–100
};

// ── Sector weights per risk profile ──
...
// ── Holding count per time horizon (UPDATED 7–15) ──
...
// ── Sector rationale by risk profile ──
function sectorRationale(
  sector: string,
  risk: RiskTolerance,
  pct: number
): string {
  ...
  const rationales: Record<RiskTolerance, Record<string, string>> = {
    conservative: {
      "Consumer Staples":
        "Defensive backbone — earnings stay stable through recessions. Companies sell what people buy regardless of economic conditions (food, household goods).",
      "Health Care":
        "Resilient demand — healthcare spending grows regardless of the economic cycle. Aging demographics provide a long-term tailwind.",
      Healthcare:
        "Resilient demand — healthcare spending grows regardless of the economic cycle. Aging demographics provide a long-term tailwind.",
      Utilities:
        "Steady dividends and regulated cash flows — among the most predictable earnings in the market.",
      Financials:
        "Banks and insurers benefit from steady interest income; provides ballast without excessive growth risk.",
      "Information Technology":
        "Limited tech exposure for a conservative portfolio — measured growth without high volatility.",
      Technology:
        "Limited tech exposure for a conservative portfolio — measured growth without high volatility.",
    },
    moderate: {
      "Information Technology":
        "Largest weight — tech drives long-term S&P 500 returns. Balanced exposure to platform giants and software leaders.",
      Technology:
        "Largest weight — tech drives long-term S&P 500 returns. Balanced exposure to platform giants and software leaders.",
      "Health Care":
        "Strong secular growth from demographics, plus defensive properties when markets turn.",
      Healthcare:
        "Strong secular growth from demographics, plus defensive properties when markets turn.",
      Financials:
        "Cyclical kicker — banks outperform during economic expansions, providing upside without speculation.",
      "Consumer Discretionary":
        "Captures upside from consumer spending — tilts portfolio toward growth without committing fully.",
      "Communication Services":
        "Mix of platform monopolies and content giants — secular growth with defensive cash flows.",
    },
    aggressive: {
      "Information Technology":
        "Highest weight — tech delivers the strongest long-term compound returns. AI, cloud, and platforms remain in early innings.",
      Technology:
        "Highest weight — tech delivers the strongest long-term compound returns. AI, cloud, and platforms remain in early innings.",
      "Consumer Discretionary":
        "Maximum exposure to consumer spending growth — captures bull markets and economic expansion most aggressively.",
      "Communication Services":
        "Platform giants and content leaders capturing digital ad dollars and streaming subscribers.",
      "Health Care":
        "Even an aggressive portfolio benefits from healthcare's secular tailwinds.",
      Healthcare:
        "Even an aggressive portfolio benefits from healthcare's secular tailwinds.",
    },
  };
  ...
  if (role === "Defensive") {
    return heavy
      ? "Defensive sector — provides stability and downside protection."
      : "Small defensive position to balance higher-risk exposures.";
  }
  if (role === "Growth") {
    return heavy
      ? "Growth-oriented sector — aims for above-market capital appreciation."
      : meaningful
        ? "Moderate growth exposure to participate in market upside."
        : "Limited growth exposure aligned with risk profile.";
  }
  if (role === "Income") {
    return "Provides dividend income and reduces overall portfolio volatility.";
  }
  return "Balanced exposure to broad economic activity.";
}

// ── Overall portfolio strategy narrative ──
function buildStrategy(
  ...
  const riskText =
    risk === "conservative"
      ? "capital preservation with steady, lower-risk growth"
      : risk === "moderate"
        ? "balanced growth combining stable earners with growth leaders"
...
function expectedReturnEstimate(
  risk: RiskTolerance,
  avgScore: number
): number {
  // Baseline by risk profile
  const baseline =
    risk === "conservative" ? 7 : risk === "moderate" ? 9 : 12;

  // Bump for higher AI scores
  const scoreBoost = (avgScore / 10000) * 3; // 0–3% bump
...
function riskNarrative(risk: RiskTolerance, horizon: TimeHorizon): string {
  if (risk === "conservative") {
    return "Lower expected drawdowns (typically −10% to −15% in market corrections). Suitable if you'd lose sleep over a 25%+ paper loss.";
  }
  if (risk === "moderate") {
    return "Moderate drawdowns expected (−15% to −25% in corrections). Balanced for investors comfortable with normal market volatility.";
  }
...
function diversificationScore(
  ...
  // Penalise concentration — top sector >40% loses points
  // Reward sector count + holding count
  ...
  return Math.max(
    0,
    Math.min(100, Math.round(sectorScore + holdingScore + 15 - concentrationPenalty))
  );
}

function buildHoldingReasoning(
  stock: { rank: number; score: number; sector: string; ticker: string },
  risk: RiskTolerance
): string {
  const parts: string[] = [];
...
export async function generatePortfolio({
  ...
  // Sort sectors by priority weight
  ...
  // Step 1: Pick best stock from each priority sector
  ...
  // Step 2: For larger portfolios, add a SECOND pick from the heaviest-weighted sectors
  if (picked.size < targetCount) {
    for (const [sectorName, weight] of sortedSectors) {
      if (picked.size >= targetCount) break;
      // Get all stocks in this sector that aren't picked yet
      const sectorPicks = stocksData.filter(
        (s) =>
          s.sector === sectorName &&
          s.ticker &&
          !picked.has(s.ticker as string) &&
          Number.isFinite(Number(s.price)) &&
          Number(s.price) > 0
      );
      const second = sectorPicks[0]; // highest-ranked unpicked in this sector
      if (second) {
        picked.set(second.ticker as string, {
          ticker: second.ticker as string,
          company: (second.company as string) ?? second.ticker,
          sector: second.sector as string,
          rank: Number(second.rank) || 999,
          score: Number(second.score) || 0,
          price: Number(second.price),
          sectorWeight: weight * 0.6, // half weight for the second pick
        });
      }
    }
  }

  // Step 3: Final fallback — add top-ranked stocks if still short
  if (picked.size < targetCount) {
  ...
  const rawWeights = candidates.map((c) => {
    const scoreFactor = 0.5 + (c.score / 10000) * 0.5;
  ...
  // Sector breakdown with rationale
  ...
  const sectorBreakdown: SectorBreakdown[] = Object.entries(sectorMap)
    .map(([sector, pct]) => ({
      ...
      rationale: sectorRationale(sector, riskTolerance, pct),
      role: sectorRole(sector),
    }))
  ...
  return {
    ...
    expectedAnnualReturn: expectedReturnEstimate(riskTolerance, avgScore),
