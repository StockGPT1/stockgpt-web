...
const { resolveTradeOrder, roundTradeMoney, roundTradeShares } = await import("../lib/trade-calculator.ts");
const { derivePortfolioHoldingAction } = await import("../lib/portfolio-action-engine.ts");
...
function simulateTrim(holding, orderInput) {
  ...
  const realisedPnl = roundTradeMoney(order.shares * (order.price - holding.entryPrice));
...
assertResolvedTrade({ value: "900", price: "90", shares: "" }, { value: 900, price: 90, shares: 10 });
assertResolvedTrade({ value: "200", price: "", shares: "1.25" }, { value: 200, price: 160, shares: 1.25 });
assertResolvedTrade({ value: "", price: "45", shares: "4" }, { value: 180, price: 45, shares: 4 });
assertResolvedTrade({ value: "100.01", price: "33.336666", shares: "3" }, { value: 100.01, price: 33.336666, shares: 3 });
assert.match(resolveTradeOrder({ value: "100", price: "45", shares: "4" }).error ?? "", /do not match/);
assert.match(resolveTradeOrder({ value: "100", price: "0", shares: "" }).error ?? "", /Enter any two/);
...
assert.equal(simulateTrim({ shares: 10, entryPrice: 100 }, { value: "100", price: "45", shares: "4" }).success, false);
...
assert.equal(
  derivePortfolioHoldingAction(baseHolding(), { riskTolerance: "moderate", cashBalance: 250, nowMs: now }).action,
  "none",
);
assert.equal(
...
assert.equal(
  derivePortfolioHoldingAction(baseHolding({ score: 5500, rank: 200, currentAllocationPct: 38, targetAllocationPct: 18 }), {
    riskTolerance: "moderate",
    cashBalance: 500,
...
function classifyOpportunityRule({
  ...
  if (held) {
    ...
    if (currentAllocation < targetAllocation - 1 && cashDrag >= 2 && exposure <= sectorCap * 0.9) {
      return "Add-more candidate";
    }
    return null;
  }
  if (heldWeak && score >= heldScore + 900 && exposure <= sectorCap * 1.15) return "Alternative to review";
  if (exposure <= 3 && score >= 7200) return "Diversification fit";
  if (score >= 8000 && rank <= 50 && exposure <= sectorCap * 0.6) return "High-conviction fit";
  if (exposure > sectorCap * 0.75) return null;
  return "Watchlist idea";
}

assert.notEqual(classifyOpportunityRule({ held: true, heldWeak: false }), "Diversification fit");
assert.equal(classifyOpportunityRule({ held: true, heldWeak: false }), "Add-more candidate");
assert.equal(classifyOpportunityRule({ held: true, heldWeak: true }), "Review existing holding");
assert.equal(classifyOpportunityRule({ held: false, exposure: 42, score: 7800 }), null);
assert.equal(classifyOpportunityRule({ held: false, exposure: 1, score: 7600 }), "Diversification fit");
assert.equal(
  classifyOpportunityRule({ held: false, heldWeak: true, heldScore: 6100, score: 7300, exposure: 18 }),
  "Alternative to review",
);
assert.equal(classifyOpportunityRule({ held: false, exposure: 12, score: 8200, rank: 30 }), "High-conviction fit");
assert.equal(classifyOpportunityRule({ fresh: false, score: 8200, rank: 20 }), null);
assert.notEqual(
...
assert.doesNotMatch(chartHealthSource, /meaningfulPointCount\s*>\s*0\s*&&\s*flat/);
...
assert.match(dashboardPortfolioSource, /export async function buildPortfolioOpportunities/);
...
const commandCentreSource = readSource("components/PortfolioCommandCentreRevolut.tsx");
assert.match(commandCentreSource, /PortfolioOpportunitiesWidget/);
assert.match(commandCentreSource, /grid-cols-\[minmax\(190px,1\.35fr\).*minmax\(112px,auto\)\]/s);
...
const sharedWidgetSource = readSource("components/PortfolioOpportunitiesWidget.tsx");
assert.match(sharedWidgetSource, /variant\?: "dashboard" \| "portfolio"/);
assert.match(sharedWidgetSource, /grid-cols-\[34px_minmax\(0,1fr\)\]/);
assert.doesNotMatch(sharedWidgetSource, /overflow-y-auto/);
